...
class white_balance
{
	...
	private static $table = 'white_balance';
	public static $data = array();
	public $white_balance_id;
	public $component_id;
	public $component_id_int;
	public $white_balance_type_id;
	public $youser_id;
	public $timestamp;
	...
	public function Get($white_balance_id = null)
	{
		if($white_balance_id != null)
		{
			self::$white_balance_id = $white_balance_id;
		}
	...
	{
		$description['white_balance_type_id'] = 'white_balance_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM white_balance  LEFT JOIN white_balance_type ON white_balance.white_balance_type_id = white_balance_type.white_balance_type_id  LEFT JOIN white_balance AS t ON white_balance.white_balance_id = t.white_balance_id WHERE white_balance.component_id = ? ORDER BY white_balance.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['white_balance_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['white_balance_type_id'] = 'multiple';
		$description['youser_id'] = 'notshown';
		$description['timestamp'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM white_balance  LEFT JOIN white_balance_type ON white_balance.white_balance_type_id = white_balance_type.white_balance_type_id  LEFT JOIN white_balance AS t ON white_balance.white_balance_id = t.white_balance_id WHERE white_balance.component_id = ? AND white_balance.timestamp = (SELECT MAX(timestamp) FROM white_balance WHERE white_balance.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($white_balance_id = null, $component_id = null, $component_id_int = null, $white_balance_type_id = null, $youser_id = null, $timestamp = null)
	{
		if($white_balance_id !== null)
		{
			self::$white_balance_id = $white_balance_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES white_balance WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO white_balance (white_balance_id, component_id, component_id_int, white_balance_type_id, youser_id, timestamp) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE white_balance_id=VALUES(white_balance_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),white_balance_type_id=VALUES(white_balance_type_id),youser_id=VALUES(youser_id),timestamp=VALUES(timestamp);", $this->white_balance_id, $this->component_id, $this->component_id_int, $this->white_balance_type_id, $this->youser_id, $this->timestamp);
