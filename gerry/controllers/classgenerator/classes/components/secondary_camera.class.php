<?php
class secondary_camera
{

	protected static $instance = null;
	private static $table = 'secondary_camera';
	public static $data = array();
	public $secondary_camera;
	public $component_id;
	public $component_id_int;
	public $secondary_camera_resolution_in_mp;
	public $timestamp;
	public $youser_id;
	...
	public function Get($secondary_camera = null)
	{
		if($secondary_camera != null)
		{
	...
	{
		$description['secondary_camera_resolution_in_mp'] = 'secondary_camera_resolution_in_mp';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM secondary_camera  WHERE secondary_camera.component_id = ? ORDER BY secondary_camera.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['secondary_camera'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['secondary_camera_resolution_in_mp'] = 'text';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM secondary_camera  WHERE secondary_camera.component_id = ? ORDER BY secondary_camera.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($secondary_camera = null, $component_id = null, $component_id_int = null, $secondary_camera_resolution_in_mp = null, $timestamp = null, $youser_id = null)
	{
		if($secondary_camera !== null)
		{
			self::$secondary_camera = $secondary_camera;
		...
		}		if($secondary_camera_resolution_in_mp !== null)
		{
			self::$secondary_camera_resolution_in_mp = $secondary_camera_resolution_in_mp;
		}		if($timestamp !== null)
		{
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES secondary_camera WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO secondary_camera (secondary_camera, component_id, component_id_int, secondary_camera_resolution_in_mp, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE secondary_camera=VALUES(secondary_camera),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),secondary_camera_resolution_in_mp=VALUES(secondary_camera_resolution_in_mp),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->secondary_camera, $this->component_id, $this->component_id_int, $this->secondary_camera_resolution_in_mp, $this->timestamp, $this->youser_id);
