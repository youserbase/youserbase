...
class camera_chip
{
	...
	private static $table = 'camera_chip';
	public static $data = array();
	public $camera_chip_id;
	public $component_id;
	public $component_id_int;
	public $camera_chip_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($camera_chip_id = null)
	{
		if($camera_chip_id != null)
		{
			self::$camera_chip_id = $camera_chip_id;
		}
	...
	{
		$description['camera_chip_type_id'] = 'camera_chip_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM camera_chip  LEFT JOIN camera_chip_type ON camera_chip.camera_chip_type_id = camera_chip_type.camera_chip_type_id  LEFT JOIN camera_chip AS t ON camera_chip.camera_chip_id = t.camera_chip_id WHERE camera_chip.component_id = ? ORDER BY camera_chip.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['camera_chip_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['camera_chip_type_id'] = 'select';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM camera_chip  LEFT JOIN camera_chip_type ON camera_chip.camera_chip_type_id = camera_chip_type.camera_chip_type_id  LEFT JOIN camera_chip AS t ON camera_chip.camera_chip_id = t.camera_chip_id WHERE camera_chip.component_id = ? ORDER BY camera_chip.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($camera_chip_id = null, $component_id = null, $component_id_int = null, $camera_chip_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($camera_chip_id !== null)
		{
			self::$camera_chip_id = $camera_chip_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES camera_chip WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO camera_chip (camera_chip_id, component_id, component_id_int, camera_chip_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE camera_chip_id=VALUES(camera_chip_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),camera_chip_type_id=VALUES(camera_chip_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->camera_chip_id, $this->component_id, $this->component_id_int, $this->camera_chip_type_id, $this->timestamp, $this->youser_id);
