...
class sound_output
{
	...
	private static $table = 'sound_output';
	public static $data = array();
	public $sound_output_id;
	public $component_id;
	public $sound_output_type_id;
	public $youser_id;
	public $timestamp;
	...
	public function Get($sound_output_id = null)
	{
		if($sound_output_id != null)
		{
			self::$sound_output_id = $sound_output_id;
		}
	...
	{
		$description['sound_output_type_id'] = 'sound_output_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM sound_output  LEFT JOIN sound_output_type ON sound_output.sound_output_type_id = sound_output_type.sound_output_type_id  LEFT JOIN sound_output AS t ON sound_output.sound_output_id = t.sound_output_id WHERE sound_output.component_id = ? ORDER BY sound_output.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['sound_output_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['sound_output_type_id'] = 'multiple';
		$description['youser_id'] = 'notshown';
		$description['timestamp'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM sound_output  LEFT JOIN sound_output_type ON sound_output.sound_output_type_id = sound_output_type.sound_output_type_id  LEFT JOIN sound_output AS t ON sound_output.sound_output_id = t.sound_output_id WHERE sound_output.component_id = ? AND sound_output.timestamp = (SELECT MAX(timestamp) FROM sound_output WHERE sound_output.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($sound_output_id = null, $component_id = null, $sound_output_type_id = null, $youser_id = null, $timestamp = null)
	{
		if($sound_output_id !== null)
		{
			self::$sound_output_id = $sound_output_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES sound_output WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO sound_output (sound_output_id, component_id, sound_output_type_id, youser_id, timestamp) VALUES(?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE sound_output_id=VALUES(sound_output_id),component_id=VALUES(component_id),sound_output_type_id=VALUES(sound_output_type_id),youser_id=VALUES(youser_id),timestamp=VALUES(timestamp);", $this->sound_output_id, $this->component_id, $this->sound_output_type_id, $this->youser_id, $this->timestamp);
