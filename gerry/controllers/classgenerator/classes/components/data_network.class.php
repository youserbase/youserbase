...
class data_network
{
	...
	private static $table = 'data_network';
	public static $data = array();
	public $data_network_id;
	public $component_id;
	...
	public function Get($data_network_id = null)
	{
		if($data_network_id != null)
		{
			self::$data_network_id = $data_network_id;
		}
	...
	public function Loadview($component_id = null, $shift = 0)
	{
		...
		$response = DBManager::Get('devices')->query("SELECT * FROM data_network  LEFT JOIN data_network_type ON data_network.data_network_type_id = data_network_type.data_network_type_id  LEFT JOIN data_network AS t ON data_network.data_network_id = t.data_network_id WHERE data_network.component_id = ? ORDER BY data_network.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['data_network_id'] = 'hidden';
		...
		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM data_network  LEFT JOIN data_network_type ON data_network.data_network_type_id = data_network_type.data_network_type_id  LEFT JOIN data_network AS t ON data_network.data_network_id = t.data_network_id WHERE data_network.component_id = ? AND data_network.timestamp = (SELECT MAX(timestamp) FROM data_network WHERE data_network.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($data_network_id = null, $component_id = null, $component_id_int = null, $data_network_type_id = null, $data_network_type_id_int = null, $timestamp = null, $youser_id = null)
	{
		if($data_network_id !== null)
		{
			self::$data_network_id = $data_network_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES data_network WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO data_network (data_network_id, component_id, component_id_int, data_network_type_id, data_network_type_id_int, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE data_network_id=VALUES(data_network_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),data_network_type_id=VALUES(data_network_type_id),data_network_type_id_int=VALUES(data_network_type_id_int),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->data_network_id, $this->component_id, $this->component_id_int, $this->data_network_type_id, $this->data_network_type_id_int, $this->timestamp, $this->youser_id);
