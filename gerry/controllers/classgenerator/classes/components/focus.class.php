...
class focus
{
	...
	private static $table = 'focus';
	public static $data = array();
	public $focus_id;
	public $component_id;
	public $component_id_int;
	public $focus_type_id;
	public $focus_type_id_int;
	public $timestamp;
	public $youser_id;
	...
	public function Get($focus_id = null)
	{
		if($focus_id != null)
		{
	...
	{
		$description['focus_type_id'] = 'focus_type_id';
		$description['focus_type_id_int'] = 'focus_type_id_int';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM focus  LEFT JOIN focus_type ON focus.focus_type_id = focus_type.focus_type_id  LEFT JOIN focus AS t ON focus.focus_id = t.focus_id WHERE focus.component_id = ? ORDER BY focus.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['focus_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['focus_type_id'] = 'multiple';
		$description['focus_type_id_int'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM focus  LEFT JOIN focus_type ON focus.focus_type_id = focus_type.focus_type_id  LEFT JOIN focus AS t ON focus.focus_id = t.focus_id WHERE focus.component_id = ? AND focus.timestamp = (SELECT MAX(timestamp) FROM focus WHERE focus.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($focus_id = null, $component_id = null, $component_id_int = null, $focus_type_id = null, $focus_type_id_int = null, $timestamp = null, $youser_id = null)
	{
		if($focus_id !== null)
		{
			self::$focus_id = $focus_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES focus WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO focus (focus_id, component_id, component_id_int, focus_type_id, focus_type_id_int, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE focus_id=VALUES(focus_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),focus_type_id=VALUES(focus_type_id),focus_type_id_int=VALUES(focus_type_id_int),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->focus_id, $this->component_id, $this->component_id_int, $this->focus_type_id, $this->focus_type_id_int, $this->timestamp, $this->youser_id);
