...
class input
{
	...
	public function Get($input_id = null)
	{
		if($input_id != null)
		{
	...
	public function Loadview($component_id = null, $shift = 0)
	{
		...
		$response = DBManager::Get('devices')->query("SELECT * FROM input  LEFT JOIN color_type ON input.color_type_id = color_type.color_type_id  LEFT JOIN input AS t ON input.input_id = t.input_id WHERE input.component_id = ? ORDER BY input.timestamp DESC;", $component_id)->to_array();
	...
	public function Loadedit($component_id = null)
	{
		...
		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM input  LEFT JOIN color_type ON input.color_type_id = color_type.color_type_id  LEFT JOIN input AS t ON input.input_id = t.input_id WHERE input.component_id = ? AND input.timestamp = (SELECT MAX(timestamp) FROM input WHERE input.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($input_id = null, $component_id = null, $component_id_int = null, $color_type_id = null, $color_type_id_int = null, $timestamp = null, $youser_id = null)
	{
		if($input_id !== null)
		{
			self::$input_id = $input_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES input WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO input (input_id, component_id, component_id_int, color_type_id, color_type_id_int, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE input_id=VALUES(input_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),color_type_id=VALUES(color_type_id),color_type_id_int=VALUES(color_type_id_int),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->input_id, $this->component_id, $this->component_id_int, $this->color_type_id, $this->color_type_id_int, $this->timestamp, $this->youser_id);
