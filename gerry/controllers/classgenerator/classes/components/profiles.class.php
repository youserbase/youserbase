...
class profiles
{
	...
	public function Get($profiles_id = null)
	{
		if($profiles_id != null)
		{
	...
	{
		$description['profiles_type_id'] = 'profiles_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM profiles  LEFT JOIN profiles_type ON profiles.profiles_type_id = profiles_type.profiles_type_id  LEFT JOIN profiles AS t ON profiles.profiles_id = t.profiles_id WHERE profiles.component_id = ? ORDER BY profiles.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['profiles_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['profiles_type_id'] = 'multiple';
		$description['youser_id'] = 'notshown';
		$description['timestamp'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM profiles  LEFT JOIN profiles_type ON profiles.profiles_type_id = profiles_type.profiles_type_id  LEFT JOIN profiles AS t ON profiles.profiles_id = t.profiles_id WHERE profiles.component_id = ? AND profiles.timestamp = (SELECT MAX(timestamp) FROM profiles WHERE profiles.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($profiles_id = null, $component_id = null, $component_id_int = null, $profiles_type_id = null, $youser_id = null, $timestamp = null)
	{
		if($profiles_id !== null)
		{
			self::$profiles_id = $profiles_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES profiles WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO profiles (profiles_id, component_id, component_id_int, profiles_type_id, youser_id, timestamp) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE profiles_id=VALUES(profiles_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),profiles_type_id=VALUES(profiles_type_id),youser_id=VALUES(youser_id),timestamp=VALUES(timestamp);", $this->profiles_id, $this->component_id, $this->component_id_int, $this->profiles_type_id, $this->youser_id, $this->timestamp);
