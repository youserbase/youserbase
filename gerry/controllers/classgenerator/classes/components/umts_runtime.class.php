...
class umts_runtime
{
	...
	private static $table = 'umts_runtime';
	public static $data = array();
	public $umts_runtime_id;
	public $component_id;
	...
	public function Get($umts_runtime_id = null)
	{
		if($umts_runtime_id != null)
		{
			self::$umts_runtime_id = $umts_runtime_id;
		}
	...
	public function Loadview($component_id = null, $shift = 0)
	{
		...
		$response = DBManager::Get('devices')->query("SELECT * FROM umts_runtime  LEFT JOIN time_units_type ON umts_runtime.time_units_type_id = time_units_type.time_units_type_id  LEFT JOIN umts_runtime AS t ON umts_runtime.umts_runtime_id = t.umts_runtime_id WHERE umts_runtime.component_id = ? ORDER BY umts_runtime.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['umts_runtime_id'] = 'hidden';
		...
		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM umts_runtime  LEFT JOIN time_units_type ON umts_runtime.time_units_type_id = time_units_type.time_units_type_id  LEFT JOIN umts_runtime AS t ON umts_runtime.umts_runtime_id = t.umts_runtime_id WHERE umts_runtime.component_id = ? ORDER BY umts_runtime.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($umts_runtime_id = null, $component_id = null, $component_id_int = null, $talktime = null, $standbytime = null, $time_units_type_id = null, $youser_id = null, $timestamp = null)
	{
		if($umts_runtime_id !== null)
		{
			self::$umts_runtime_id = $umts_runtime_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES umts_runtime WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO umts_runtime (umts_runtime_id, component_id, component_id_int, talktime, standbytime, time_units_type_id, youser_id, timestamp) VALUES(?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE umts_runtime_id=VALUES(umts_runtime_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),talktime=VALUES(talktime),standbytime=VALUES(standbytime),time_units_type_id=VALUES(time_units_type_id),youser_id=VALUES(youser_id),timestamp=VALUES(timestamp);", $this->umts_runtime_id, $this->component_id, $this->component_id_int, $this->talktime, $this->standbytime, $this->time_units_type_id, $this->youser_id, $this->timestamp);
