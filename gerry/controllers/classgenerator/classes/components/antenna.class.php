...
class antenna
{
	...
	private static $table = 'antenna';
	public static $data = array();
	public $antenna_id;
	public $antenna_id_int;
	public $component_id;
	public $component_id_int;
	public $antenna_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($antenna_id = null)
	{
		if($antenna_id != null)
		{
	...
	{
		$description['antenna_type_id'] = 'antenna_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM antenna  LEFT JOIN antenna_type ON antenna.antenna_type_id = antenna_type.antenna_type_id  LEFT JOIN antenna AS t ON antenna.antenna_id = t.antenna_id WHERE antenna.component_id = ? ORDER BY antenna.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['antenna_id'] = 'hidden';
		$description['antenna_id_int'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['antenna_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM antenna  LEFT JOIN antenna_type ON antenna.antenna_type_id = antenna_type.antenna_type_id  LEFT JOIN antenna AS t ON antenna.antenna_id = t.antenna_id WHERE antenna.component_id = ? AND antenna.timestamp = (SELECT MAX(timestamp) FROM antenna WHERE antenna.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($antenna_id = null, $antenna_id_int = null, $component_id = null, $component_id_int = null, $antenna_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($antenna_id !== null)
		{
			self::$antenna_id = $antenna_id;
		}		if($antenna_id_int !== null)
		{
			self::$antenna_id_int = $antenna_id_int;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES antenna WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO antenna (antenna_id, antenna_id_int, component_id, component_id_int, antenna_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE antenna_id=VALUES(antenna_id),antenna_id_int=VALUES(antenna_id_int),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),antenna_type_id=VALUES(antenna_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->antenna_id, $this->antenna_id_int, $this->component_id, $this->component_id_int, $this->antenna_type_id, $this->timestamp, $this->youser_id);
