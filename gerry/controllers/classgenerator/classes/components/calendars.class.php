...
class calendars
{
	...
	private static $table = 'calendars';
	public static $data = array();
	public $calendars_id;
	public $component_id;
	public $component_id_int;
	public $number_of_records;
	public $number_of_alarms;
	...
	public function Get($calendars_id = null)
	{
		if($calendars_id != null)
		{
			self::$calendars_id = $calendars_id;
		}
	...
	{
		$description['calendars_id'] = 'calendars_id';
		$description['number_of_records'] = 'number_of_records';
		$description['number_of_alarms'] = 'number_of_alarms';
		$description['repeated_entries'] = 'repeated_entries';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM calendars  WHERE calendars.component_id = ? ORDER BY calendars.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['calendars_id'] = '';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['number_of_records'] = 'text';
		$description['number_of_alarms'] = 'text';
		$description['repeated_entries'] = 'text';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM calendars  WHERE calendars.component_id = ? ORDER BY calendars.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($calendars_id = null, $component_id = null, $component_id_int = null, $number_of_records = null, $number_of_alarms = null, $repeated_entries = null, $timestamp = null, $youser_id = null)
	{
		if($calendars_id !== null)
		{
			self::$calendars_id = $calendars_id;
		...
		}		if($number_of_alarms !== null)
		{
			self::$number_of_alarms = $number_of_alarms;
		}		if($repeated_entries !== null)
		{
			self::$repeated_entries = $repeated_entries;
		}		if($timestamp !== null)
		{
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES calendars WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO calendars (calendars_id, component_id, component_id_int, number_of_records, number_of_alarms, repeated_entries, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE calendars_id=VALUES(calendars_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),number_of_records=VALUES(number_of_records),number_of_alarms=VALUES(number_of_alarms),repeated_entries=VALUES(repeated_entries),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->calendars_id, $this->component_id, $this->component_id_int, $this->number_of_records, $this->number_of_alarms, $this->repeated_entries, $this->timestamp, $this->youser_id);
