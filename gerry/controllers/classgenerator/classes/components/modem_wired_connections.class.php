<?php
class modem_wired_connections
{

	protected static $instance = null;
	private static $table = 'modem_wired_connections';
	public static $data = array();
	public $modem_wired_connections_id;
	public $component_id;
	public $component_id_int;
	public $data_port_type_id;
	public $data_port_type_id_int;
	public $timestamp;
	public $youser_id;
	...
	public function Get($modem_wired_connections_id = null)
	{
		if($modem_wired_connections_id != null)
		{
			self::$modem_wired_connections_id = $modem_wired_connections_id;
		}
	...
	{
		$description['data_port_type_id'] = 'data_port_type_id';
		$description['data_port_type_id_int'] = 'data_port_type_id_int';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM modem_wired_connections  LEFT JOIN data_port_type ON modem_wired_connections.data_port_type_id = data_port_type.data_port_type_id  LEFT JOIN modem_wired_connections AS t ON modem_wired_connections.modem_wired_connections_id = t.modem_wired_connections_id WHERE modem_wired_connections.component_id = ? ORDER BY modem_wired_connections.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['modem_wired_connections_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['data_port_type_id'] = 'multiple';
		$description['data_port_type_id_int'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM modem_wired_connections  LEFT JOIN data_port_type ON modem_wired_connections.data_port_type_id = data_port_type.data_port_type_id  LEFT JOIN modem_wired_connections AS t ON modem_wired_connections.modem_wired_connections_id = t.modem_wired_connections_id WHERE modem_wired_connections.component_id = ? AND modem_wired_connections.timestamp = (SELECT MAX(timestamp) FROM modem_wired_connections WHERE modem_wired_connections.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($modem_wired_connections_id = null, $component_id = null, $component_id_int = null, $data_port_type_id = null, $data_port_type_id_int = null, $timestamp = null, $youser_id = null)
	{
		if($modem_wired_connections_id !== null)
		{
			self::$modem_wired_connections_id = $modem_wired_connections_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES modem_wired_connections WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO modem_wired_connections (modem_wired_connections_id, component_id, component_id_int, data_port_type_id, data_port_type_id_int, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE modem_wired_connections_id=VALUES(modem_wired_connections_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),data_port_type_id=VALUES(data_port_type_id),data_port_type_id_int=VALUES(data_port_type_id_int),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->modem_wired_connections_id, $this->component_id, $this->component_id_int, $this->data_port_type_id, $this->data_port_type_id_int, $this->timestamp, $this->youser_id);
