...
class processor
{
	...
	private static $table = 'processor';
	public static $data = array();
	public $processor_id;
	public $component_id;
	public $component_id_int;
	public $mhz;
	public $processor_type_id;
	public $processor_type_id_int;
	public $timestamp;
	public $youser_id;
	...
	public function Get($processor_id = null)
	{
	...
	{
		$description['mhz'] = 'mhz';
		$description['processor_type_id'] = 'processor_type_id';
		$description['processor_type_id_int'] = 'processor_type_id_int';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM processor  LEFT JOIN processor_type ON processor.processor_type_id = processor_type.processor_type_id  LEFT JOIN processor AS t ON processor.processor_id = t.processor_id WHERE processor.component_id = ? ORDER BY processor.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['processor_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['mhz'] = 'text';
		$description['processor_type_id'] = 'select';
		$description['processor_type_id_int'] = 'select';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM processor  LEFT JOIN processor_type ON processor.processor_type_id = processor_type.processor_type_id  LEFT JOIN processor AS t ON processor.processor_id = t.processor_id WHERE processor.component_id = ? ORDER BY processor.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($processor_id = null, $component_id = null, $component_id_int = null, $mhz = null, $processor_type_id = null, $processor_type_id_int = null, $timestamp = null, $youser_id = null)
	{
		if($processor_id !== null)
		{
			self::$processor_id = $processor_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES processor WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO processor (processor_id, component_id, component_id_int, mhz, processor_type_id, processor_type_id_int, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE processor_id=VALUES(processor_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),mhz=VALUES(mhz),processor_type_id=VALUES(processor_type_id),processor_type_id_int=VALUES(processor_type_id_int),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->processor_id, $this->component_id, $this->component_id_int, $this->mhz, $this->processor_type_id, $this->processor_type_id_int, $this->timestamp, $this->youser_id);
