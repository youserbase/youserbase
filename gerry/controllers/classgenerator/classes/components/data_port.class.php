...
class data_port
{
	...
	private static $table = 'data_port';
	...
	public function Get($data_port_id = null)
	{
		if($data_port_id != null)
		{
			self::$data_port_id = $data_port_id;
		}
	...
	public function Loadview($component_id = null, $shift = 0)
	{
		...
		$response = DBManager::Get('devices')->query("SELECT * FROM data_port  LEFT JOIN data_port_type ON data_port.data_port_type_id = data_port_type.data_port_type_id  LEFT JOIN data_port AS t ON data_port.data_port_id = t.data_port_id WHERE data_port.component_id = ? ORDER BY data_port.timestamp DESC;", $component_id)->to_array();
			...
			foreach($content as $line_name => $line_content)
			{
				if(strpos($line_name, '_id') === false && strpos($line_name, 'units') === false && strpos($line_name, 'memory_size_type') === false && strpos($line_name, 'currency_type_name') === false && strpos($line_name,'version') === false && strpos($line_name,'device') === false && strpos($line_name,'alternative') === false && strpos($line_name,'short') === false && strpos($line_name,'website') === false)
				{
					if(preg_match('/((\w+)?(_type))(_name\b)/', $line_name, $match))
					{
						...
						{
							if($line_name == 'data_port_type_name')
							{
	...
	{
		$description['data_port_id'] = 'hidden';
		...
		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM data_port  LEFT JOIN data_port_type ON data_port.data_port_type_id = data_port_type.data_port_type_id  LEFT JOIN data_port AS t ON data_port.data_port_id = t.data_port_id WHERE data_port.component_id = ? AND data_port.timestamp = (SELECT MAX(timestamp) FROM data_port WHERE data_port.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($data_port_id = null, $component_id = null, $component_id_int = null, $data_port_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($data_port_id !== null)
		{
			self::$data_port_id = $data_port_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES data_port WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO data_port (data_port_id, component_id, component_id_int, data_port_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE data_port_id=VALUES(data_port_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),data_port_type_id=VALUES(data_port_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->data_port_id, $this->component_id, $this->component_id_int, $this->data_port_type_id, $this->timestamp, $this->youser_id);
