...
class contacts
{
	...
	public $contacts_numbers;
	public $contacts_numbers_per_contact;
	public $contacts_name_sorting_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($contacts_id = null)
	{
		if($contacts_id != null)
		{
	...
	{
		$description['contacts_numbers'] = 'contacts_numbers';
		$description['contacts_numbers_per_contact'] = 'contacts_numbers_per_contact';
		$description['contacts_name_sorting_type_id'] = 'contacts_name_sorting_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM contacts  LEFT JOIN contacts_name_sorting_type ON contacts.contacts_name_sorting_type_id = contacts_name_sorting_type.contacts_name_sorting_type_id  LEFT JOIN contacts AS t ON contacts.contacts_id = t.contacts_id WHERE contacts.component_id = ? ORDER BY contacts.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['contacts_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['contacts_numbers'] = 'text';
		$description['contacts_numbers_per_contact'] = 'text';
		$description['contacts_name_sorting_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM contacts  LEFT JOIN contacts_name_sorting_type ON contacts.contacts_name_sorting_type_id = contacts_name_sorting_type.contacts_name_sorting_type_id  LEFT JOIN contacts AS t ON contacts.contacts_id = t.contacts_id WHERE contacts.component_id = ? AND contacts.timestamp = (SELECT MAX(timestamp) FROM contacts WHERE contacts.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($contacts_id = null, $component_id = null, $component_id_int = null, $contacts_numbers = null, $contacts_numbers_per_contact = null, $contacts_name_sorting_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($contacts_id !== null)
		{
			self::$contacts_id = $contacts_id;
		...
		}		if($contacts_numbers !== null)
		{
			self::$contacts_numbers = $contacts_numbers;
		}		if($contacts_numbers_per_contact !== null)
		{
			self::$contacts_numbers_per_contact = $contacts_numbers_per_contact;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES contacts WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO contacts (contacts_id, component_id, component_id_int, contacts_numbers, contacts_numbers_per_contact, contacts_name_sorting_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE contacts_id=VALUES(contacts_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),contacts_numbers=VALUES(contacts_numbers),contacts_numbers_per_contact=VALUES(contacts_numbers_per_contact),contacts_name_sorting_type_id=VALUES(contacts_name_sorting_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->contacts_id, $this->component_id, $this->component_id_int, $this->contacts_numbers, $this->contacts_numbers_per_contact, $this->contacts_name_sorting_type_id, $this->timestamp, $this->youser_id);
