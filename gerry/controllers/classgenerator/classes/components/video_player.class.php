...
class video_player
{
	...
	private static $table = 'video_player';
	public static $data = array();
	public $video_player_id;
	public $component_id;
	...
	public function Get($video_player_id = null)
	{
		if($video_player_id != null)
		{
			self::$video_player_id = $video_player_id;
		}
	...
	public function Loadview($component_id = null, $shift = 0)
	{
		...
		$response = DBManager::Get('devices')->query("SELECT * FROM video_player  WHERE video_player.component_id = ? ORDER BY video_player.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['video_player_id'] = 'hidden';
		...
		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM video_player  WHERE video_player.component_id = ? ORDER BY video_player.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($video_player_id = null, $component_id = null, $component_id_int = null, $timestamp = null, $youser_id = null)
	{
		if($video_player_id !== null)
		{
			self::$video_player_id = $video_player_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES video_player WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO video_player (video_player_id, component_id, component_id_int, timestamp, youser_id) VALUES(?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE video_player_id=VALUES(video_player_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->video_player_id, $this->component_id, $this->component_id_int, $this->timestamp, $this->youser_id);
