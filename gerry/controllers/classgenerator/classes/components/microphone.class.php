...
class microphone
{
	...
	private static $table = 'microphone';
	public static $data = array();
	public $microphone_id;
	public $component_id;
	public $component_id_int;
	public $microphone_type_id;
	public $microphone_type_id_int;
	public $timestamp;
	public $youser_id;
	...
	public function Get($microphone_id = null)
	{
		if($microphone_id != null)
		{
	...
	{
		$description['microphone_type_id'] = 'microphone_type_id';
		$description['microphone_type_id_int'] = 'microphone_type_id_int';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM microphone  LEFT JOIN microphone_type ON microphone.microphone_type_id = microphone_type.microphone_type_id  LEFT JOIN microphone AS t ON microphone.microphone_id = t.microphone_id WHERE microphone.component_id = ? ORDER BY microphone.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['microphone_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['microphone_type_id'] = 'select';
		$description['microphone_type_id_int'] = 'select';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM microphone  LEFT JOIN microphone_type ON microphone.microphone_type_id = microphone_type.microphone_type_id  LEFT JOIN microphone AS t ON microphone.microphone_id = t.microphone_id WHERE microphone.component_id = ? ORDER BY microphone.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($microphone_id = null, $component_id = null, $component_id_int = null, $microphone_type_id = null, $microphone_type_id_int = null, $timestamp = null, $youser_id = null)
	{
		if($microphone_id !== null)
		{
			self::$microphone_id = $microphone_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES microphone WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO microphone (microphone_id, component_id, component_id_int, microphone_type_id, microphone_type_id_int, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE microphone_id=VALUES(microphone_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),microphone_type_id=VALUES(microphone_type_id),microphone_type_id_int=VALUES(microphone_type_id_int),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->microphone_id, $this->component_id, $this->component_id_int, $this->microphone_type_id, $this->microphone_type_id_int, $this->timestamp, $this->youser_id);
