...
class camera_macro
{
	...
	private static $table = 'camera_macro';
	public static $data = array();
	public $camera_macro_id;
	public $component_id;
	public $component_id_int;
	public $camera_macro_from;
	public $camera_macro_to;
	...
	public function Get($camera_macro_id = null)
	{
		if($camera_macro_id != null)
		{
			self::$camera_macro_id = $camera_macro_id;
		}
	...
	{
		$description['camera_macro_from'] = 'camera_macro_from';
		$description['camera_macro_to'] = 'camera_macro_to';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM camera_macro  LEFT JOIN size_units_type ON camera_macro.size_units_type_id = size_units_type.size_units_type_id  LEFT JOIN camera_macro AS t ON camera_macro.camera_macro_id = t.camera_macro_id WHERE camera_macro.component_id = ? ORDER BY camera_macro.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['camera_macro_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['camera_macro_from'] = 'text';
		$description['camera_macro_to'] = 'text';
		$description['size_units_type_id'] = 'notshown';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM camera_macro  LEFT JOIN size_units_type ON camera_macro.size_units_type_id = size_units_type.size_units_type_id  LEFT JOIN camera_macro AS t ON camera_macro.camera_macro_id = t.camera_macro_id WHERE camera_macro.component_id = ? ORDER BY camera_macro.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($camera_macro_id = null, $component_id = null, $component_id_int = null, $camera_macro_from = null, $camera_macro_to = null, $size_units_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($camera_macro_id !== null)
		{
			self::$camera_macro_id = $camera_macro_id;
		...
		}		if($camera_macro_from !== null)
		{
			self::$camera_macro_from = $camera_macro_from;
		}		if($camera_macro_to !== null)
		{
			self::$camera_macro_to = $camera_macro_to;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES camera_macro WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO camera_macro (camera_macro_id, component_id, component_id_int, camera_macro_from, camera_macro_to, size_units_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE camera_macro_id=VALUES(camera_macro_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),camera_macro_from=VALUES(camera_macro_from),camera_macro_to=VALUES(camera_macro_to),size_units_type_id=VALUES(size_units_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->camera_macro_id, $this->component_id, $this->component_id_int, $this->camera_macro_from, $this->camera_macro_to, $this->size_units_type_id, $this->timestamp, $this->youser_id);
