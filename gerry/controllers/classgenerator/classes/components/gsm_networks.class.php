...
class gsm_networks
{
	...
	private static $table = 'gsm_networks';
	public static $data = array();
	public $gsm_networks_id;
	public $component_id;
	public $component_id_int;
	public $gsm_networks_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($gsm_networks_id = null)
	{
		if($gsm_networks_id != null)
		{
			self::$gsm_networks_id = $gsm_networks_id;
		}
	...
	{
		$description['gsm_networks_type_id'] = 'gsm_networks_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM gsm_networks  LEFT JOIN gsm_networks_type ON gsm_networks.gsm_networks_type_id = gsm_networks_type.gsm_networks_type_id  LEFT JOIN gsm_networks AS t ON gsm_networks.gsm_networks_id = t.gsm_networks_id WHERE gsm_networks.component_id = ? ORDER BY gsm_networks.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['gsm_networks_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['gsm_networks_type_id'] = 'select';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM gsm_networks  LEFT JOIN gsm_networks_type ON gsm_networks.gsm_networks_type_id = gsm_networks_type.gsm_networks_type_id  LEFT JOIN gsm_networks AS t ON gsm_networks.gsm_networks_id = t.gsm_networks_id WHERE gsm_networks.component_id = ? ORDER BY gsm_networks.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($gsm_networks_id = null, $component_id = null, $component_id_int = null, $gsm_networks_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($gsm_networks_id !== null)
		{
			self::$gsm_networks_id = $gsm_networks_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES gsm_networks WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO gsm_networks (gsm_networks_id, component_id, component_id_int, gsm_networks_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE gsm_networks_id=VALUES(gsm_networks_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),gsm_networks_type_id=VALUES(gsm_networks_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->gsm_networks_id, $this->component_id, $this->component_id_int, $this->gsm_networks_type_id, $this->timestamp, $this->youser_id);
