...
class nav_compass
{
	...
	private static $table = 'nav_compass';
	public static $data = array();
	public $nav_compass_id;
	public $component_id;
	public $component_id_int;
	public $nav_compass_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($nav_compass_id = null)
	{
		if($nav_compass_id != null)
		{
			self::$nav_compass_id = $nav_compass_id;
		}
	...
	{
		$description['nav_compass_type_id'] = 'nav_compass_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM nav_compass  LEFT JOIN nav_compass_type ON nav_compass.nav_compass_type_id = nav_compass_type.nav_compass_type_id  LEFT JOIN nav_compass AS t ON nav_compass.nav_compass_id = t.nav_compass_id WHERE nav_compass.component_id = ? ORDER BY nav_compass.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['nav_compass_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['nav_compass_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM nav_compass  LEFT JOIN nav_compass_type ON nav_compass.nav_compass_type_id = nav_compass_type.nav_compass_type_id  LEFT JOIN nav_compass AS t ON nav_compass.nav_compass_id = t.nav_compass_id WHERE nav_compass.component_id = ? AND nav_compass.timestamp = (SELECT MAX(timestamp) FROM nav_compass WHERE nav_compass.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($nav_compass_id = null, $component_id = null, $component_id_int = null, $nav_compass_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($nav_compass_id !== null)
		{
			self::$nav_compass_id = $nav_compass_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES nav_compass WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO nav_compass (nav_compass_id, component_id, component_id_int, nav_compass_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE nav_compass_id=VALUES(nav_compass_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),nav_compass_type_id=VALUES(nav_compass_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->nav_compass_id, $this->component_id, $this->component_id_int, $this->nav_compass_type_id, $this->timestamp, $this->youser_id);
