...
class cdma_networks
{
	...
	private static $table = 'cdma_networks';
	public static $data = array();
	public $cdma_networks_id;
	public $component_id;
	public $component_id_int;
	public $cdma_networks_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($cdma_networks_id = null)
	{
		if($cdma_networks_id != null)
		{
			self::$cdma_networks_id = $cdma_networks_id;
		}
	...
	{
		$description['cdma_networks_type_id'] = 'cdma_networks_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM cdma_networks  LEFT JOIN cdma_networks_type ON cdma_networks.cdma_networks_type_id = cdma_networks_type.cdma_networks_type_id  LEFT JOIN cdma_networks AS t ON cdma_networks.cdma_networks_id = t.cdma_networks_id WHERE cdma_networks.component_id = ? ORDER BY cdma_networks.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['cdma_networks_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['cdma_networks_type_id'] = 'select';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM cdma_networks  LEFT JOIN cdma_networks_type ON cdma_networks.cdma_networks_type_id = cdma_networks_type.cdma_networks_type_id  LEFT JOIN cdma_networks AS t ON cdma_networks.cdma_networks_id = t.cdma_networks_id WHERE cdma_networks.component_id = ? ORDER BY cdma_networks.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($cdma_networks_id = null, $component_id = null, $component_id_int = null, $cdma_networks_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($cdma_networks_id !== null)
		{
			self::$cdma_networks_id = $cdma_networks_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES cdma_networks WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO cdma_networks (cdma_networks_id, component_id, component_id_int, cdma_networks_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE cdma_networks_id=VALUES(cdma_networks_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),cdma_networks_type_id=VALUES(cdma_networks_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->cdma_networks_id, $this->component_id, $this->component_id_int, $this->cdma_networks_type_id, $this->timestamp, $this->youser_id);
