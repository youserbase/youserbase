...
class umts_networks
{
	...
	private static $table = 'umts_networks';
	public static $data = array();
	public $umts_networks_id;
	public $component_id;
	public $component_id_int;
	public $umts_networks_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($umts_networks_id = null)
	{
		if($umts_networks_id != null)
		{
			self::$umts_networks_id = $umts_networks_id;
		}
	...
	{
		$description['umts_networks_type_id'] = 'umts_networks_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM umts_networks  LEFT JOIN umts_networks_type ON umts_networks.umts_networks_type_id = umts_networks_type.umts_networks_type_id  LEFT JOIN umts_networks AS t ON umts_networks.umts_networks_id = t.umts_networks_id WHERE umts_networks.component_id = ? ORDER BY umts_networks.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['umts_networks_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['umts_networks_type_id'] = 'select';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM umts_networks  LEFT JOIN umts_networks_type ON umts_networks.umts_networks_type_id = umts_networks_type.umts_networks_type_id  LEFT JOIN umts_networks AS t ON umts_networks.umts_networks_id = t.umts_networks_id WHERE umts_networks.component_id = ? ORDER BY umts_networks.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($umts_networks_id = null, $component_id = null, $component_id_int = null, $umts_networks_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($umts_networks_id !== null)
		{
			self::$umts_networks_id = $umts_networks_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES umts_networks WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO umts_networks (umts_networks_id, component_id, component_id_int, umts_networks_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE umts_networks_id=VALUES(umts_networks_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),umts_networks_type_id=VALUES(umts_networks_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->umts_networks_id, $this->component_id, $this->component_id_int, $this->umts_networks_type_id, $this->timestamp, $this->youser_id);
