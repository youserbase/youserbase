...
class predecessors
{
	...
	private static $table = 'predecessors';
	...
	public function Loadview($component_id = null, $shift = 0)
	{
		...
		$description['child_id'] = 'child_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM predecessors  WHERE predecessors.component_id = ? ORDER BY predecessors.timestamp DESC;", $component_id)->to_array();
	...
	public function Loadedit($component_id = null)
	{
		...
		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM predecessors  WHERE predecessors.component_id = ? ORDER BY predecessors.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($parent_id = null, $child_id = null, $timestamp = null, $youser_id = null)
	{
		if($parent_id !== null)
		{
			self::$parent_id = $parent_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES predecessors WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO predecessors (parent_id, child_id, timestamp, youser_id) VALUES(?, ?, ?, ?) ON DUPLICATE KEY UPDATE parent_id=VALUES(parent_id),child_id=VALUES(child_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->parent_id, $this->child_id, $this->timestamp, $this->youser_id);
