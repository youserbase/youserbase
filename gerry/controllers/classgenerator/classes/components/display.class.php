...
class display
{
	...
	public function Get($display_id = null)
	{
		if($display_id != null)
		{
	...
	public function Loadview($component_id = null, $shift = 0)
	{
		...
		$response = DBManager::Get('devices')->query("SELECT * FROM display  LEFT JOIN color_space_type ON display.color_space_type_id = color_space_type.color_space_type_id  LEFT JOIN display_type ON display.display_type_id = display_type.display_type_id  LEFT JOIN size_units_type ON display.size_units_type_id = size_units_type.size_units_type_id  LEFT JOIN display AS t ON display.display_id = t.display_id WHERE display.component_id = ? ORDER BY display.timestamp DESC;", $component_id)->to_array();
	...
	public function Loadedit($component_id = null)
	{
		...
		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM display  LEFT JOIN color_space_type ON display.color_space_type_id = color_space_type.color_space_type_id  LEFT JOIN display_type ON display.display_type_id = display_type.display_type_id  LEFT JOIN size_units_type ON display.size_units_type_id = size_units_type.size_units_type_id  LEFT JOIN display AS t ON display.display_id = t.display_id WHERE display.component_id = ? ORDER BY display.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($display_id = null, $component_id = null, $component_id_int = null, $color_space_type_id = null, $display_type_id = null, $display_resolution_x = null, $display_resolution_y = null, $display_size_diagonally = null, $size_units_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($display_id !== null)
		{
			self::$display_id = $display_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES display WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO display (display_id, component_id, component_id_int, color_space_type_id, display_type_id, display_resolution_x, display_resolution_y, display_size_diagonally, size_units_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE display_id=VALUES(display_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),color_space_type_id=VALUES(color_space_type_id),display_type_id=VALUES(display_type_id),display_resolution_x=VALUES(display_resolution_x),display_resolution_y=VALUES(display_resolution_y),display_size_diagonally=VALUES(display_size_diagonally),size_units_type_id=VALUES(size_units_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->display_id, $this->component_id, $this->component_id_int, $this->color_space_type_id, $this->display_type_id, $this->display_resolution_x, $this->display_resolution_y, $this->display_size_diagonally, $this->size_units_type_id, $this->timestamp, $this->youser_id);
