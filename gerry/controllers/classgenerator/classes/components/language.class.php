...
class language
{
	...
	public function Get($language_id = null)
	{
		if($language_id != null)
		{
	...
	{
		$description['language_name'] = 'language_name';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM language  WHERE language.component_id = ? ORDER BY language.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['language_id'] = 'hidden';
		$description['language_name'] = 'text';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM language  WHERE language.component_id = ? ORDER BY language.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($language_id = null, $language_name = null, $timestamp = null, $youser_id = null)
	{
		if($language_id !== null)
		{
			self::$language_id = $language_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES language WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO language (language_id, language_name, timestamp, youser_id) VALUES(?, ?, ?, ?) ON DUPLICATE KEY UPDATE language_id=VALUES(language_id),language_name=VALUES(language_name),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->language_id, $this->language_name, $this->timestamp, $this->youser_id);
