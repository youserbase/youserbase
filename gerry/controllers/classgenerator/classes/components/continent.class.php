...
class continent
{
	...
	private static $table = 'continent';
	...
	public function Get($continent_id = null)
	{
	...
	{
		$description['continent_name'] = 'continent_name';
		$description['youser_id'] = 'youser_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM continent  WHERE continent.component_id = ? ORDER BY continent.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['continent_id'] = 'hidden';
		$description['continent_name'] = 'text';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = '';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM continent  WHERE continent.component_id = ? ORDER BY continent.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($continent_id = null, $continent_name = null, $timestamp = null, $youser_id = null)
	{
		if($continent_id !== null)
		{
			self::$continent_id = $continent_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES continent WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO continent (continent_id, continent_name, timestamp, youser_id) VALUES(?, ?, ?, ?) ON DUPLICATE KEY UPDATE continent_id=VALUES(continent_id),continent_name=VALUES(continent_name),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->continent_id, $this->continent_name, $this->timestamp, $this->youser_id);
