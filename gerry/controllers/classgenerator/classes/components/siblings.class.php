...
class siblings
{
	...
	public function Get($brother_id = null)
	{
		if($brother_id != null)
		{
	...
	public function Loadview($component_id = null, $shift = 0)
	{
		...
		$response = DBManager::Get('devices')->query("SELECT * FROM siblings  WHERE siblings.component_id = ? ORDER BY siblings.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['brother_id'] = 'hidden';
		$description['sister_id'] = 'hidden';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM siblings  WHERE siblings.component_id = ? ORDER BY siblings.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($brother_id = null, $sister_id = null, $timestamp = null, $youser_id = null)
	{
		if($brother_id !== null)
		{
			self::$brother_id = $brother_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES siblings WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO siblings (brother_id, sister_id, timestamp, youser_id) VALUES(?, ?, ?, ?) ON DUPLICATE KEY UPDATE brother_id=VALUES(brother_id),sister_id=VALUES(sister_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->brother_id, $this->sister_id, $this->timestamp, $this->youser_id);
