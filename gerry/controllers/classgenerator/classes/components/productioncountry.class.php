...
class productioncountry
{
	...
	private static $table = 'productioncountry';
	...
	public function Get($pc_id = null)
	{
		if($pc_id != null)
		{
			self::$pc_id = $pc_id;
	...
	{
		$description['pc_id'] = 'pc_id';
		$description['manufacturer_id'] = 'manufacturer_id';
		$description['country_id'] = 'country_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM productioncountry  LEFT JOIN manufacturer ON productioncountry.manufacturer_Id = manufacturer.manufacturer_id  LEFT JOIN productioncountry AS t ON productioncountry.productioncountry_id = t.productioncountry_id WHERE productioncountry.component_id = ? ORDER BY productioncountry.timestamp DESC;", $component_id)->to_array();
	...
	public function Loadedit($component_id = null)
	{
		...
		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM productioncountry  WHERE productioncountry.component_id = ? ORDER BY productioncountry.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($pc_id = null, $manufacturer_id = null, $country_id = null)
	{
		if($pc_id !== null)
		{
			self::$pc_id = $pc_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES productioncountry WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO productioncountry (pc_id, manufacturer_id, country_id) VALUES(?, ?, ?) ON DUPLICATE KEY UPDATE pc_id=VALUES(pc_id),manufacturer_id=VALUES(manufacturer_id),country_id=VALUES(country_id);", $this->pc_id, $this->manufacturer_id, $this->country_id);
