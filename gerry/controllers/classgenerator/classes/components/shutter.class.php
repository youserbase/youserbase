...
class shutter
{
	...
	public function Get($shutter_id = null)
	{
		if($shutter_id != null)
		{
	...
	{
		$description['shutter_speed'] = 'shutter_speed';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM shutter  LEFT JOIN time_units_type ON shutter.time_units_type_id = time_units_type.time_units_type_id  LEFT JOIN shutter AS t ON shutter.shutter_id = t.shutter_id WHERE shutter.component_id = ? ORDER BY shutter.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['shutter_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['shutter_speed'] = 'text';
		$description['time_units_type_id'] = 'notshown';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM shutter  LEFT JOIN time_units_type ON shutter.time_units_type_id = time_units_type.time_units_type_id  LEFT JOIN shutter AS t ON shutter.shutter_id = t.shutter_id WHERE shutter.component_id = ? ORDER BY shutter.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($shutter_id = null, $component_id = null, $component_id_int = null, $shutter_speed = null, $time_units_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($shutter_id !== null)
		{
			self::$shutter_id = $shutter_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES shutter WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO shutter (shutter_id, component_id, component_id_int, shutter_speed, time_units_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE shutter_id=VALUES(shutter_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),shutter_speed=VALUES(shutter_speed),time_units_type_id=VALUES(time_units_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->shutter_id, $this->component_id, $this->component_id_int, $this->shutter_speed, $this->time_units_type_id, $this->timestamp, $this->youser_id);
