...
class finder
{
	...
	private static $table = 'finder';
	public static $data = array();
	public $finder_id;
	public $component_id;
	public $component_id_int;
	public $finder_type_id;
	public $finder_type_id_int;
	public $timestamp;
	public $youser_id;
	...
	public function Get($finder_id = null)
	{
		if($finder_id != null)
		{
	...
	{
		$description['finder_type_id'] = 'finder_type_id';
		$description['finder_type_id_int'] = 'finder_type_id_int';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM finder  LEFT JOIN finder_type ON finder.finder_type_id = finder_type.finder_type_id  LEFT JOIN finder AS t ON finder.finder_id = t.finder_id WHERE finder.component_id = ? ORDER BY finder.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['finder_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['finder_type_id'] = 'multiple';
		$description['finder_type_id_int'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM finder  LEFT JOIN finder_type ON finder.finder_type_id = finder_type.finder_type_id  LEFT JOIN finder AS t ON finder.finder_id = t.finder_id WHERE finder.component_id = ? AND finder.timestamp = (SELECT MAX(timestamp) FROM finder WHERE finder.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($finder_id = null, $component_id = null, $component_id_int = null, $finder_type_id = null, $finder_type_id_int = null, $timestamp = null, $youser_id = null)
	{
		if($finder_id !== null)
		{
			self::$finder_id = $finder_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES finder WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO finder (finder_id, component_id, component_id_int, finder_type_id, finder_type_id_int, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE finder_id=VALUES(finder_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),finder_type_id=VALUES(finder_type_id),finder_type_id_int=VALUES(finder_type_id_int),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->finder_id, $this->component_id, $this->component_id_int, $this->finder_type_id, $this->finder_type_id_int, $this->timestamp, $this->youser_id);
