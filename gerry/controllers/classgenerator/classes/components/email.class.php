...
class email
{
	...
	private static $table = 'email';
	public static $data = array();
	public $email_id;
	public $component_id;
	public $component_id_int;
	public $number_of_accounts;
	public $max_size_off_attachement;
	...
	public function Get($email_id = null)
	{
		if($email_id != null)
		{
	...
	{
		$description['number_of_accounts'] = 'number_of_accounts';
		$description['max_size_off_attachement'] = 'max_size_off_attachement';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM email  LEFT JOIN memory_size_type ON email.memory_size_type_id = memory_size_type.memory_size_type_id  LEFT JOIN email AS t ON email.email_id = t.email_id WHERE email.component_id = ? ORDER BY email.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['email_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['number_of_accounts'] = 'text';
		$description['max_size_off_attachement'] = 'text';
		$description['memory_size_type_id'] = 'notshown';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM email  LEFT JOIN memory_size_type ON email.memory_size_type_id = memory_size_type.memory_size_type_id  LEFT JOIN email AS t ON email.email_id = t.email_id WHERE email.component_id = ? ORDER BY email.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($email_id = null, $component_id = null, $component_id_int = null, $number_of_accounts = null, $max_size_off_attachement = null, $memory_size_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($email_id !== null)
		{
			self::$email_id = $email_id;
		...
		}		if($number_of_accounts !== null)
		{
			self::$number_of_accounts = $number_of_accounts;
		}		if($max_size_off_attachement !== null)
		{
			self::$max_size_off_attachement = $max_size_off_attachement;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES email WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO email (email_id, component_id, component_id_int, number_of_accounts, max_size_off_attachement, memory_size_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE email_id=VALUES(email_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),number_of_accounts=VALUES(number_of_accounts),max_size_off_attachement=VALUES(max_size_off_attachement),memory_size_type_id=VALUES(memory_size_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->email_id, $this->component_id, $this->component_id_int, $this->number_of_accounts, $this->max_size_off_attachement, $this->memory_size_type_id, $this->timestamp, $this->youser_id);
