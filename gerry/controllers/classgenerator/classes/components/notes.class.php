...
class notes
{
	...
	public function Get($notes_id = null)
	{
		if($notes_id != null)
		{
	...
	{
		$description['notes_max_number'] = 'notes_max_number';
		$description['sendvia_type_id'] = 'sendvia_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM notes  LEFT JOIN sendvia_type ON notes.sendvia_type_id = sendvia_type.sendvia_type_id  LEFT JOIN notes AS t ON notes.notes_id = t.notes_id WHERE notes.component_id = ? ORDER BY notes.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['notes_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['notes_max_number'] = 'text';
		$description['sendvia_type_id'] = 'multiple';
		$description['youser_id'] = 'notshown';
		$description['timestamp'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM notes  LEFT JOIN sendvia_type ON notes.sendvia_type_id = sendvia_type.sendvia_type_id  LEFT JOIN notes AS t ON notes.notes_id = t.notes_id WHERE notes.component_id = ? AND notes.timestamp = (SELECT MAX(timestamp) FROM notes WHERE notes.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($notes_id = null, $component_id = null, $component_id_int = null, $notes_max_number = null, $sendvia_type_id = null, $youser_id = null, $timestamp = null)
	{
		if($notes_id !== null)
		{
			self::$notes_id = $notes_id;
		...
		}		if($notes_max_number !== null)
		{
			self::$notes_max_number = $notes_max_number;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES notes WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO notes (notes_id, component_id, component_id_int, notes_max_number, sendvia_type_id, youser_id, timestamp) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE notes_id=VALUES(notes_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),notes_max_number=VALUES(notes_max_number),sendvia_type_id=VALUES(sendvia_type_id),youser_id=VALUES(youser_id),timestamp=VALUES(timestamp);", $this->notes_id, $this->component_id, $this->component_id_int, $this->notes_max_number, $this->sendvia_type_id, $this->youser_id, $this->timestamp);
