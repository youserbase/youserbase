<?php
class notes_functions
{

	protected static $instance = null;
	private static $table = 'notes_functions';
	public static $data = array();
	public $notes_functions_id;
	public $component_id;
	public $component_id_int;
	public $notes_functions_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($notes_functions_id = null)
	{
		if($notes_functions_id != null)
		{
			self::$notes_functions_id = $notes_functions_id;
		}
	...
	{
		$description['notes_functions_type_id'] = 'notes_functions_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM notes_functions  LEFT JOIN notes_functions_type ON notes_functions.notes_functions_type_id = notes_functions_type.notes_functions_type_id  LEFT JOIN notes_functions AS t ON notes_functions.notes_functions_id = t.notes_functions_id WHERE notes_functions.component_id = ? ORDER BY notes_functions.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['notes_functions_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['notes_functions_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM notes_functions  LEFT JOIN notes_functions_type ON notes_functions.notes_functions_type_id = notes_functions_type.notes_functions_type_id  LEFT JOIN notes_functions AS t ON notes_functions.notes_functions_id = t.notes_functions_id WHERE notes_functions.component_id = ? AND notes_functions.timestamp = (SELECT MAX(timestamp) FROM notes_functions WHERE notes_functions.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($notes_functions_id = null, $component_id = null, $component_id_int = null, $notes_functions_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($notes_functions_id !== null)
		{
			self::$notes_functions_id = $notes_functions_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES notes_functions WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO notes_functions (notes_functions_id, component_id, component_id_int, notes_functions_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE notes_functions_id=VALUES(notes_functions_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),notes_functions_type_id=VALUES(notes_functions_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->notes_functions_id, $this->component_id, $this->component_id_int, $this->notes_functions_type_id, $this->timestamp, $this->youser_id);
