...
class services
{
	...
	public function Loadview($component_id = null, $shift = 0)
	{
		...
		$response = DBManager::Get('devices')->query("SELECT * FROM services  WHERE services.component_id = ? ORDER BY services.timestamp DESC;", $component_id)->to_array();
	...
	public function Loadedit($component_id = null)
	{
		...
		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM services  WHERE services.component_id = ? ORDER BY services.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($service_name = null, $status = null)
	{
		if($service_name !== null)
		{
			self::$service_name = $service_name;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES services WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO services (service_name, status) VALUES(?, ?) ON DUPLICATE KEY UPDATE service_name=VALUES(service_name),status=VALUES(status);", $this->service_name, $this->status);
