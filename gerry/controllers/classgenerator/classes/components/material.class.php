...
class material
{
	...
	public function Get($material_id = null)
	{
		if($material_id != null)
		{
	...
	{
		$description['material_type_id'] = 'material_type_id';
		$description['material_type_id_int'] = 'material_type_id_int';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM material  LEFT JOIN material_type ON material.material_type_id = material_type.material_type_id  LEFT JOIN material AS t ON material.material_id = t.material_id WHERE material.component_id = ? ORDER BY material.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['material_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['material_type_id'] = 'multiple';
		$description['material_type_id_int'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM material  LEFT JOIN material_type ON material.material_type_id = material_type.material_type_id  LEFT JOIN material AS t ON material.material_id = t.material_id WHERE material.component_id = ? AND material.timestamp = (SELECT MAX(timestamp) FROM material WHERE material.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($material_id = null, $component_id = null, $component_id_int = null, $material_type_id = null, $material_type_id_int = null, $timestamp = null, $youser_id = null)
	{
		if($material_id !== null)
		{
			self::$material_id = $material_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES material WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO material (material_id, component_id, component_id_int, material_type_id, material_type_id_int, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE material_id=VALUES(material_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),material_type_id=VALUES(material_type_id),material_type_id_int=VALUES(material_type_id_int),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->material_id, $this->component_id, $this->component_id_int, $this->material_type_id, $this->material_type_id_int, $this->timestamp, $this->youser_id);
