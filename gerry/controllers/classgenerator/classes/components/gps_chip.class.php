...
class gps_chip
{
	...
	private static $table = 'gps_chip';
	public static $data = array();
	public $gps_chip_id;
	public $component_id;
	public $component_id_int;
	public $gps_chip_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($gps_chip_id = null)
	{
		if($gps_chip_id != null)
		{
	...
	{
		$description['gps_chip_type_id'] = 'gps_chip_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM gps_chip  LEFT JOIN gps_chip_type ON gps_chip.gps_chip_type_id = gps_chip_type.gps_chip_type_id  LEFT JOIN gps_chip AS t ON gps_chip.gps_chip_id = t.gps_chip_id WHERE gps_chip.component_id = ? ORDER BY gps_chip.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['gps_chip_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['gps_chip_type_id'] = 'select';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM gps_chip  LEFT JOIN gps_chip_type ON gps_chip.gps_chip_type_id = gps_chip_type.gps_chip_type_id  LEFT JOIN gps_chip AS t ON gps_chip.gps_chip_id = t.gps_chip_id WHERE gps_chip.component_id = ? ORDER BY gps_chip.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($gps_chip_id = null, $component_id = null, $component_id_int = null, $gps_chip_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($gps_chip_id !== null)
		{
			self::$gps_chip_id = $gps_chip_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES gps_chip WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO gps_chip (gps_chip_id, component_id, component_id_int, gps_chip_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE gps_chip_id=VALUES(gps_chip_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),gps_chip_type_id=VALUES(gps_chip_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->gps_chip_id, $this->component_id, $this->component_id_int, $this->gps_chip_type_id, $this->timestamp, $this->youser_id);
