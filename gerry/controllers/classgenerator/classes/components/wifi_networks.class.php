...
class wifi_networks
{
	...
	private static $table = 'wifi_networks';
	public static $data = array();
	public $wifi_networks_id;
	public $component_id;
	public $component_id_int;
	public $wifi_networks_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($wifi_networks_id = null)
	{
		if($wifi_networks_id != null)
		{
			self::$wifi_networks_id = $wifi_networks_id;
		}
	...
	{
		$description['wifi_networks_type_id'] = 'wifi_networks_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM wifi_networks  LEFT JOIN wifi_networks_type ON wifi_networks.wifi_networks_type_id = wifi_networks_type.wifi_networks_type_id  LEFT JOIN wifi_networks AS t ON wifi_networks.wifi_networks_id = t.wifi_networks_id WHERE wifi_networks.component_id = ? ORDER BY wifi_networks.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['wifi_networks_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['wifi_networks_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM wifi_networks  LEFT JOIN wifi_networks_type ON wifi_networks.wifi_networks_type_id = wifi_networks_type.wifi_networks_type_id  LEFT JOIN wifi_networks AS t ON wifi_networks.wifi_networks_id = t.wifi_networks_id WHERE wifi_networks.component_id = ? AND wifi_networks.timestamp = (SELECT MAX(timestamp) FROM wifi_networks WHERE wifi_networks.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($wifi_networks_id = null, $component_id = null, $component_id_int = null, $wifi_networks_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($wifi_networks_id !== null)
		{
			self::$wifi_networks_id = $wifi_networks_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES wifi_networks WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO wifi_networks (wifi_networks_id, component_id, component_id_int, wifi_networks_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE wifi_networks_id=VALUES(wifi_networks_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),wifi_networks_type_id=VALUES(wifi_networks_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->wifi_networks_id, $this->component_id, $this->component_id_int, $this->wifi_networks_type_id, $this->timestamp, $this->youser_id);
