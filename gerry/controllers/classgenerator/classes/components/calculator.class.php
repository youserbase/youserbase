...
class calculator
{
	...
	private static $table = 'calculator';
	public static $data = array();
	public $calculator_id;
	public $component_id;
	public $component_id_int;
	public $calculator_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($calculator_id = null)
	{
	...
	{
		$description['calculator_type_id'] = 'calculator_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM calculator  LEFT JOIN calculator_type ON calculator.calculator_type_id = calculator_type.calculator_type_id  LEFT JOIN calculator AS t ON calculator.calculator_id = t.calculator_id WHERE calculator.component_id = ? ORDER BY calculator.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['calculator_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['calculator_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM calculator  LEFT JOIN calculator_type ON calculator.calculator_type_id = calculator_type.calculator_type_id  LEFT JOIN calculator AS t ON calculator.calculator_id = t.calculator_id WHERE calculator.component_id = ? AND calculator.timestamp = (SELECT MAX(timestamp) FROM calculator WHERE calculator.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($calculator_id = null, $component_id = null, $component_id_int = null, $calculator_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($calculator_id !== null)
		{
			self::$calculator_id = $calculator_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES calculator WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO calculator (calculator_id, component_id, component_id_int, calculator_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE calculator_id=VALUES(calculator_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),calculator_type_id=VALUES(calculator_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->calculator_id, $this->component_id, $this->component_id_int, $this->calculator_type_id, $this->timestamp, $this->youser_id);
