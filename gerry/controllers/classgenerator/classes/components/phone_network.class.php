...
class phone_network
{
	...
	private static $table = 'phone_network';
	public static $data = array();
	public $phone_network_id;
	public $component_id;
	public $component_id_int;
	public $phone_network_type_id;
	public $phone_network_type_id_int;
	public $timestamp;
	public $youser_id;
	...
	public function Get($phone_network_id = null)
	{
		if($phone_network_id != null)
		{
			self::$phone_network_id = $phone_network_id;
		}
	...
	{
		$description['phone_network_type_id'] = 'phone_network_type_id';
		$description['phone_network_type_id_int'] = 'phone_network_type_id_int';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM phone_network  LEFT JOIN phone_network_type ON phone_network.phone_network_type_id = phone_network_type.phone_network_type_id  LEFT JOIN phone_network AS t ON phone_network.phone_network_id = t.phone_network_id WHERE phone_network.component_id = ? ORDER BY phone_network.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['phone_network_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['phone_network_type_id'] = 'multiple';
		$description['phone_network_type_id_int'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM phone_network  LEFT JOIN phone_network_type ON phone_network.phone_network_type_id = phone_network_type.phone_network_type_id  LEFT JOIN phone_network AS t ON phone_network.phone_network_id = t.phone_network_id WHERE phone_network.component_id = ? AND phone_network.timestamp = (SELECT MAX(timestamp) FROM phone_network WHERE phone_network.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($phone_network_id = null, $component_id = null, $component_id_int = null, $phone_network_type_id = null, $phone_network_type_id_int = null, $timestamp = null, $youser_id = null)
	{
		if($phone_network_id !== null)
		{
			self::$phone_network_id = $phone_network_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES phone_network WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO phone_network (phone_network_id, component_id, component_id_int, phone_network_type_id, phone_network_type_id_int, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE phone_network_id=VALUES(phone_network_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),phone_network_type_id=VALUES(phone_network_type_id),phone_network_type_id_int=VALUES(phone_network_type_id_int),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->phone_network_id, $this->component_id, $this->component_id_int, $this->phone_network_type_id, $this->phone_network_type_id_int, $this->timestamp, $this->youser_id);
