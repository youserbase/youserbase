...
class scene_mode
{
	...
	private static $table = 'scene_mode';
	public static $data = array();
	public $scene_mode_id;
	public $component_id;
	...
	public function Get($scene_mode_id = null)
	{
		if($scene_mode_id != null)
		{
			self::$scene_mode_id = $scene_mode_id;
		}
	...
	public function Loadview($component_id = null, $shift = 0)
	{
		...
		$response = DBManager::Get('devices')->query("SELECT * FROM scene_mode  LEFT JOIN scene_mode_type ON scene_mode.scene_mode_type_id = scene_mode_type.scene_mode_type_id  LEFT JOIN scene_mode AS t ON scene_mode.scene_mode_id = t.scene_mode_id WHERE scene_mode.component_id = ? ORDER BY scene_mode.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['scene_mode_id'] = 'hidden';
		...
		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM scene_mode  LEFT JOIN scene_mode_type ON scene_mode.scene_mode_type_id = scene_mode_type.scene_mode_type_id  LEFT JOIN scene_mode AS t ON scene_mode.scene_mode_id = t.scene_mode_id WHERE scene_mode.component_id = ? AND scene_mode.timestamp = (SELECT MAX(timestamp) FROM scene_mode WHERE scene_mode.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($scene_mode_id = null, $component_id = null, $component_id_int = null, $scene_mode_type_id = null, $scene_mode_number_of = null, $youser_id = null, $timestamp = null)
	{
		if($scene_mode_id !== null)
		{
			self::$scene_mode_id = $scene_mode_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES scene_mode WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO scene_mode (scene_mode_id, component_id, component_id_int, scene_mode_type_id, scene_mode_number_of, youser_id, timestamp) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE scene_mode_id=VALUES(scene_mode_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),scene_mode_type_id=VALUES(scene_mode_type_id),scene_mode_number_of=VALUES(scene_mode_number_of),youser_id=VALUES(youser_id),timestamp=VALUES(timestamp);", $this->scene_mode_id, $this->component_id, $this->component_id_int, $this->scene_mode_type_id, $this->scene_mode_number_of, $this->youser_id, $this->timestamp);
