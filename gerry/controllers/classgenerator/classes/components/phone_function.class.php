...
class phone_function
{
	...
	private static $table = 'phone_function';
	public static $data = array();
	public $phone_function_id;
	public $component_id;
	public $component_id_int;
	public $phone_function_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($phone_function_id = null)
	{
		if($phone_function_id != null)
		{
			self::$phone_function_id = $phone_function_id;
		}
	...
	{
		$description['phone_function_type_id'] = 'phone_function_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM phone_function  LEFT JOIN phone_function_type ON phone_function.phone_function_type_id = phone_function_type.phone_function_type_id  LEFT JOIN phone_function AS t ON phone_function.phone_function_id = t.phone_function_id WHERE phone_function.component_id = ? ORDER BY phone_function.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['phone_function_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['phone_function_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM phone_function  LEFT JOIN phone_function_type ON phone_function.phone_function_type_id = phone_function_type.phone_function_type_id  LEFT JOIN phone_function AS t ON phone_function.phone_function_id = t.phone_function_id WHERE phone_function.component_id = ? AND phone_function.timestamp = (SELECT MAX(timestamp) FROM phone_function WHERE phone_function.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($phone_function_id = null, $component_id = null, $component_id_int = null, $phone_function_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($phone_function_id !== null)
		{
			self::$phone_function_id = $phone_function_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES phone_function WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO phone_function (phone_function_id, component_id, component_id_int, phone_function_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE phone_function_id=VALUES(phone_function_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),phone_function_type_id=VALUES(phone_function_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->phone_function_id, $this->component_id, $this->component_id_int, $this->phone_function_type_id, $this->timestamp, $this->youser_id);
