...
class build_in
{
	...
	private static $table = 'build_in';
	public static $data = array();
	public $build_in_id;
	public $device_id;
	public $device_id_int;
	public $component_name;
	public $build_in_status;
	public $youser_id;
	public $timestamp;
	...
	public function Get($build_in_id = null)
	{
		if($build_in_id != null)
		{
			self::$build_in_id = $build_in_id;
		}
	...
	public function Loadview($component_id = null, $shift = 0)
	{
		...
		$response = DBManager::Get('devices')->query("SELECT * FROM build_in  WHERE build_in.component_id = ? ORDER BY build_in.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['build_in_id'] = 'hidden';
		$description['device_id'] = 'hidden';
		$description['device_id_int'] = 'hidden';
		$description['component_name'] = 'hidden';
		$description['build_in_status'] = 'hidden';
		$description['youser_id'] = 'notshown';
		$description['timestamp'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM build_in  WHERE build_in.component_id = ? ORDER BY build_in.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($build_in_id = null, $device_id = null, $device_id_int = null, $component_name = null, $build_in_status = null, $youser_id = null, $timestamp = null)
	{
		if($build_in_id !== null)
		{
			self::$build_in_id = $build_in_id;
		...
		}		if($build_in_status !== null)
		{
			self::$build_in_status = $build_in_status;
		}		if($youser_id !== null)
		{
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES build_in WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO build_in (build_in_id, device_id, device_id_int, component_name, build_in_status, youser_id, timestamp) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE build_in_id=VALUES(build_in_id),device_id=VALUES(device_id),device_id_int=VALUES(device_id_int),component_name=VALUES(component_name),build_in_status=VALUES(build_in_status),youser_id=VALUES(youser_id),timestamp=VALUES(timestamp);", $this->build_in_id, $this->device_id, $this->device_id_int, $this->component_name, $this->build_in_status, $this->youser_id, $this->timestamp);
