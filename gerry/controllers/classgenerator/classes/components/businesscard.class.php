...
class businesscard
{
	...
	private static $table = 'businesscard';
	public static $data = array();
	public $businesscard_id;
	public $component_id;
	public $component_id_int;
	public $businesscard_type_id;
	public $youser_id;
	public $timestamp;
	...
	public function Get($businesscard_id = null)
	{
		if($businesscard_id != null)
		{
			self::$businesscard_id = $businesscard_id;
		}
	...
	{
		$description['businesscard_type_id'] = 'businesscard_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM businesscard  LEFT JOIN businesscard_type ON businesscard.businesscard_type_id = businesscard_type.businesscard_type_id  LEFT JOIN businesscard AS t ON businesscard.businesscard_id = t.businesscard_id WHERE businesscard.component_id = ? ORDER BY businesscard.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['businesscard_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['businesscard_type_id'] = 'multiple';
		$description['youser_id'] = 'notshown';
		$description['timestamp'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM businesscard  LEFT JOIN businesscard_type ON businesscard.businesscard_type_id = businesscard_type.businesscard_type_id  LEFT JOIN businesscard AS t ON businesscard.businesscard_id = t.businesscard_id WHERE businesscard.component_id = ? AND businesscard.timestamp = (SELECT MAX(timestamp) FROM businesscard WHERE businesscard.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($businesscard_id = null, $component_id = null, $component_id_int = null, $businesscard_type_id = null, $youser_id = null, $timestamp = null)
	{
		if($businesscard_id !== null)
		{
			self::$businesscard_id = $businesscard_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES businesscard WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO businesscard (businesscard_id, component_id, component_id_int, businesscard_type_id, youser_id, timestamp) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE businesscard_id=VALUES(businesscard_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),businesscard_type_id=VALUES(businesscard_type_id),youser_id=VALUES(youser_id),timestamp=VALUES(timestamp);", $this->businesscard_id, $this->component_id, $this->component_id_int, $this->businesscard_type_id, $this->youser_id, $this->timestamp);
