...
class clock
{
	...
	private static $table = 'clock';
	public static $data = array();
	public $clock_id;
	public $component_id;
	public $component_id_int;
	public $clock_features;
	public $alarm_features;
	public $clock_type;
	public $timestamp;
	public $youser_id;
	...
	public function Get($clock_id = null)
	{
		if($clock_id != null)
		{
	...
	{
		$description['clock_features'] = 'clock_features';
		$description['alarm_features'] = 'alarm_features';
		$description['clock_type'] = 'clock_type';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM clock  WHERE clock.component_id = ? ORDER BY clock.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['clock_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['clock_features'] = 'multiple';
		$description['alarm_features'] = 'multiple';
		$description['clock_type'] = 'select';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM clock  WHERE clock.component_id = ? AND clock.timestamp = (SELECT MAX(timestamp) FROM clock WHERE clock.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($clock_id = null, $component_id = null, $component_id_int = null, $clock_features = null, $alarm_features = null, $clock_type = null, $timestamp = null, $youser_id = null)
	{
		if($clock_id !== null)
		{
			self::$clock_id = $clock_id;
		...
		}		if($clock_features !== null)
		{
			self::$clock_features = $clock_features;
		}		if($alarm_features !== null)
		{
			self::$alarm_features = $alarm_features;
		}		if($clock_type !== null)
		{
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES clock WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO clock (clock_id, component_id, component_id_int, clock_features, alarm_features, clock_type, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE clock_id=VALUES(clock_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),clock_features=VALUES(clock_features),alarm_features=VALUES(alarm_features),clock_type=VALUES(clock_type),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->clock_id, $this->component_id, $this->component_id_int, $this->clock_features, $this->alarm_features, $this->clock_type, $this->timestamp, $this->youser_id);
