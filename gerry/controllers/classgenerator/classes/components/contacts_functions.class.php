...
class contacts_functions
{
	...
	private static $table = 'contacts_functions';
	public static $data = array();
	public $contacts_functions_id;
	public $component_id;
	public $component_id_int;
	public $contacts_functions_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($contacts_functions_id = null)
	{
		if($contacts_functions_id != null)
		{
			self::$contacts_functions_id = $contacts_functions_id;
		}
	...
	{
		$description['contacts_functions_type_id'] = 'contacts_functions_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM contacts_functions  LEFT JOIN contacts_functions_type ON contacts_functions.contacts_functions_type_id = contacts_functions_type.contacts_functions_type_id  LEFT JOIN contacts_functions AS t ON contacts_functions.contacts_functions_id = t.contacts_functions_id WHERE contacts_functions.component_id = ? ORDER BY contacts_functions.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['contacts_functions_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['contacts_functions_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM contacts_functions  LEFT JOIN contacts_functions_type ON contacts_functions.contacts_functions_type_id = contacts_functions_type.contacts_functions_type_id  LEFT JOIN contacts_functions AS t ON contacts_functions.contacts_functions_id = t.contacts_functions_id WHERE contacts_functions.component_id = ? AND contacts_functions.timestamp = (SELECT MAX(timestamp) FROM contacts_functions WHERE contacts_functions.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($contacts_functions_id = null, $component_id = null, $component_id_int = null, $contacts_functions_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($contacts_functions_id !== null)
		{
			self::$contacts_functions_id = $contacts_functions_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES contacts_functions WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO contacts_functions (contacts_functions_id, component_id, component_id_int, contacts_functions_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE contacts_functions_id=VALUES(contacts_functions_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),contacts_functions_type_id=VALUES(contacts_functions_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->contacts_functions_id, $this->component_id, $this->component_id_int, $this->contacts_functions_type_id, $this->timestamp, $this->youser_id);
