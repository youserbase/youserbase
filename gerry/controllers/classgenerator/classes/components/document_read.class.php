...
class document_read
{
	...
	private static $table = 'document_read';
	public static $data = array();
	public $document_read_id;
	public $component_id;
	...
	public function Get($document_read_id = null)
	{
		if($document_read_id != null)
		{
			self::$document_read_id = $document_read_id;
		}
	...
	public function Loadview($component_id = null, $shift = 0)
	{
		...
		$response = DBManager::Get('devices')->query("SELECT * FROM document_read  LEFT JOIN document_format_type ON document_read.document_format_type_id = document_format_type.document_format_type_id  LEFT JOIN document_read AS t ON document_read.document_read_id = t.document_read_id WHERE document_read.component_id = ? ORDER BY document_read.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['document_read_id'] = 'hidden';
		...
		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM document_read  LEFT JOIN document_format_type ON document_read.document_format_type_id = document_format_type.document_format_type_id  LEFT JOIN document_read AS t ON document_read.document_read_id = t.document_read_id WHERE document_read.component_id = ? AND document_read.timestamp = (SELECT MAX(timestamp) FROM document_read WHERE document_read.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($document_read_id = null, $component_id = null, $component_id_int = null, $document_format_type_id = null, $youser_id = null, $timestamp = null)
	{
		if($document_read_id !== null)
		{
			self::$document_read_id = $document_read_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES document_read WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO document_read (document_read_id, component_id, component_id_int, document_format_type_id, youser_id, timestamp) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE document_read_id=VALUES(document_read_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),document_format_type_id=VALUES(document_format_type_id),youser_id=VALUES(youser_id),timestamp=VALUES(timestamp);", $this->document_read_id, $this->component_id, $this->component_id_int, $this->document_format_type_id, $this->youser_id, $this->timestamp);
