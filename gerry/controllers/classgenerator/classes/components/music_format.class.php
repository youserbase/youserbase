...
class music_format
{
	...
	private static $table = 'music_format';
	public static $data = array();
	public $music_format_id;
	public $component_id;
	...
	public function Get($music_format_id = null)
	{
		if($music_format_id != null)
		{
			self::$music_format_id = $music_format_id;
		}
	...
	public function Loadview($component_id = null, $shift = 0)
	{
		...
		$response = DBManager::Get('devices')->query("SELECT * FROM music_format  LEFT JOIN music_format_type ON music_format.music_format_type_id = music_format_type.music_format_type_id  LEFT JOIN music_format AS t ON music_format.music_format_id = t.music_format_id WHERE music_format.component_id = ? ORDER BY music_format.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['music_format_id'] = 'hidden';
		...
		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM music_format  LEFT JOIN music_format_type ON music_format.music_format_type_id = music_format_type.music_format_type_id  LEFT JOIN music_format AS t ON music_format.music_format_id = t.music_format_id WHERE music_format.component_id = ? AND music_format.timestamp = (SELECT MAX(timestamp) FROM music_format WHERE music_format.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($music_format_id = null, $component_id = null, $component_id_int = null, $music_format_type_id = null, $music_format_type_id_int = null, $timestamp = null, $youser_id = null)
	{
		if($music_format_id !== null)
		{
			self::$music_format_id = $music_format_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES music_format WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO music_format (music_format_id, component_id, component_id_int, music_format_type_id, music_format_type_id_int, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE music_format_id=VALUES(music_format_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),music_format_type_id=VALUES(music_format_type_id),music_format_type_id_int=VALUES(music_format_type_id_int),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->music_format_id, $this->component_id, $this->component_id_int, $this->music_format_type_id, $this->music_format_type_id_int, $this->timestamp, $this->youser_id);
