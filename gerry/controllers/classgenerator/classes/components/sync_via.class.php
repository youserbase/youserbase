...
class sync_via
{
	...
	private static $table = 'sync_via';
	public static $data = array();
	public $sync_via;
	public $component_id;
	public $component_id_int;
	public $sync_via_type_id;
	public $youser_id;
	public $timestamp;
	...
	public function Get($sync_via = null)
	{
		if($sync_via != null)
		{
			self::$sync_via = $sync_via;
		}
	...
	{
		$description['sync_via_type_id'] = 'sync_via_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM sync_via  LEFT JOIN sync_via_type ON sync_via.sync_via_type_id = sync_via_type.sync_via_type_id  LEFT JOIN sync_via AS t ON sync_via.sync_via_id = t.sync_via_id WHERE sync_via.component_id = ? ORDER BY sync_via.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['sync_via'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['sync_via_type_id'] = 'multiple';
		$description['youser_id'] = 'notshown';
		$description['timestamp'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM sync_via  LEFT JOIN sync_via_type ON sync_via.sync_via_type_id = sync_via_type.sync_via_type_id  LEFT JOIN sync_via AS t ON sync_via.sync_via_id = t.sync_via_id WHERE sync_via.component_id = ? AND sync_via.timestamp = (SELECT MAX(timestamp) FROM sync_via WHERE sync_via.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($sync_via = null, $component_id = null, $component_id_int = null, $sync_via_type_id = null, $youser_id = null, $timestamp = null)
	{
		if($sync_via !== null)
		{
			self::$sync_via = $sync_via;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES sync_via WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO sync_via (sync_via, component_id, component_id_int, sync_via_type_id, youser_id, timestamp) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE sync_via=VALUES(sync_via),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),sync_via_type_id=VALUES(sync_via_type_id),youser_id=VALUES(youser_id),timestamp=VALUES(timestamp);", $this->sync_via, $this->component_id, $this->component_id_int, $this->sync_via_type_id, $this->youser_id, $this->timestamp);
