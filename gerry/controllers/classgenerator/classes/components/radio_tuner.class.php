...
class radio_tuner
{
	...
	private static $table = 'radio_tuner';
	public static $data = array();
	public $radio_tuner_id;
	public $component_id;
	public $component_id_int;
	public $number_of_stations;
	public $radio_frequency_from;
	public $radio_frequency_to;
	public $timestamp;
	public $youser_id;
	...
	public function Get($radio_tuner_id = null)
	{
		if($radio_tuner_id != null)
		{
			self::$radio_tuner_id = $radio_tuner_id;
		}
	...
	{
		$description['number_of_stations'] = 'number_of_stations';
		$description['radio_frequency_from'] = 'radio_frequency_from';
		$description['radio_frequency_to'] = 'radio_frequency_to';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM radio_tuner  WHERE radio_tuner.component_id = ? ORDER BY radio_tuner.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['radio_tuner_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['number_of_stations'] = 'text';
		$description['radio_frequency_from'] = 'text';
		$description['radio_frequency_to'] = 'text';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM radio_tuner  WHERE radio_tuner.component_id = ? ORDER BY radio_tuner.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($radio_tuner_id = null, $component_id = null, $component_id_int = null, $number_of_stations = null, $radio_frequency_from = null, $radio_frequency_to = null, $timestamp = null, $youser_id = null)
	{
		if($radio_tuner_id !== null)
		{
			self::$radio_tuner_id = $radio_tuner_id;
		...
		}		if($number_of_stations !== null)
		{
			self::$number_of_stations = $number_of_stations;
		}		if($radio_frequency_from !== null)
		{
			self::$radio_frequency_from = $radio_frequency_from;
		}		if($radio_frequency_to !== null)
		{
			self::$radio_frequency_to = $radio_frequency_to;
		}		if($timestamp !== null)
		{
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES radio_tuner WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO radio_tuner (radio_tuner_id, component_id, component_id_int, number_of_stations, radio_frequency_from, radio_frequency_to, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE radio_tuner_id=VALUES(radio_tuner_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),number_of_stations=VALUES(number_of_stations),radio_frequency_from=VALUES(radio_frequency_from),radio_frequency_to=VALUES(radio_frequency_to),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->radio_tuner_id, $this->component_id, $this->component_id_int, $this->number_of_stations, $this->radio_frequency_from, $this->radio_frequency_to, $this->timestamp, $this->youser_id);
