...
class voice_codec
{
	...
	private static $table = 'voice_codec';
	public static $data = array();
	public $voice_codec_id;
	public $component_id;
	public $component_id_int;
	public $voice_codec_type_id;
	public $voice_codec_type_id_int;
	public $timestamp;
	public $youser_id;
	...
	public function Get($voice_codec_id = null)
	{
		if($voice_codec_id != null)
		{
			self::$voice_codec_id = $voice_codec_id;
		}
	...
	{
		$description['voice_codec_type_id'] = 'voice_codec_type_id';
		$description['voice_codec_type_id_int'] = 'voice_codec_type_id_int';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM voice_codec  LEFT JOIN voice_codec_type ON voice_codec.voice_codec_type_id = voice_codec_type.voice_codec_type_id  LEFT JOIN voice_codec AS t ON voice_codec.voice_codec_id = t.voice_codec_id WHERE voice_codec.component_id = ? ORDER BY voice_codec.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['voice_codec_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['voice_codec_type_id'] = 'multiple';
		$description['voice_codec_type_id_int'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM voice_codec  LEFT JOIN voice_codec_type ON voice_codec.voice_codec_type_id = voice_codec_type.voice_codec_type_id  LEFT JOIN voice_codec AS t ON voice_codec.voice_codec_id = t.voice_codec_id WHERE voice_codec.component_id = ? AND voice_codec.timestamp = (SELECT MAX(timestamp) FROM voice_codec WHERE voice_codec.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($voice_codec_id = null, $component_id = null, $component_id_int = null, $voice_codec_type_id = null, $voice_codec_type_id_int = null, $timestamp = null, $youser_id = null)
	{
		if($voice_codec_id !== null)
		{
			self::$voice_codec_id = $voice_codec_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES voice_codec WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO voice_codec (voice_codec_id, component_id, component_id_int, voice_codec_type_id, voice_codec_type_id_int, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE voice_codec_id=VALUES(voice_codec_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),voice_codec_type_id=VALUES(voice_codec_type_id),voice_codec_type_id_int=VALUES(voice_codec_type_id_int),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->voice_codec_id, $this->component_id, $this->component_id_int, $this->voice_codec_type_id, $this->voice_codec_type_id_int, $this->timestamp, $this->youser_id);
