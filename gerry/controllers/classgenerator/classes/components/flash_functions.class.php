<?php
class flash_functions
{

	protected static $instance = null;
	private static $table = 'flash_functions';
	public static $data = array();
	public $flash_functions_id;
	public $component_id;
	public $component_id_int;
	public $flash_functions_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($flash_functions_id = null)
	{
		if($flash_functions_id != null)
		{
			self::$flash_functions_id = $flash_functions_id;
		}
	...
	{
		$description['flash_functions_type_id'] = 'flash_functions_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM flash_functions  LEFT JOIN flash_functions_type ON flash_functions.flash_functions_type_id = flash_functions_type.flash_functions_type_id  LEFT JOIN flash_functions AS t ON flash_functions.flash_functions_id = t.flash_functions_id WHERE flash_functions.component_id = ? ORDER BY flash_functions.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['flash_functions_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['flash_functions_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM flash_functions  LEFT JOIN flash_functions_type ON flash_functions.flash_functions_type_id = flash_functions_type.flash_functions_type_id  LEFT JOIN flash_functions AS t ON flash_functions.flash_functions_id = t.flash_functions_id WHERE flash_functions.component_id = ? AND flash_functions.timestamp = (SELECT MAX(timestamp) FROM flash_functions WHERE flash_functions.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($flash_functions_id = null, $component_id = null, $component_id_int = null, $flash_functions_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($flash_functions_id !== null)
		{
			self::$flash_functions_id = $flash_functions_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES flash_functions WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO flash_functions (flash_functions_id, component_id, component_id_int, flash_functions_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE flash_functions_id=VALUES(flash_functions_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),flash_functions_type_id=VALUES(flash_functions_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->flash_functions_id, $this->component_id, $this->component_id_int, $this->flash_functions_type_id, $this->timestamp, $this->youser_id);
