...
class netbook
{
	...
	private static $table = 'netbook';
	public static $data = array();
	public $netbook_id;
	public $component_id;
	public $netbook_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($netbook_id = null)
	{
		if($netbook_id != null)
		{
			self::$netbook_id = $netbook_id;
		}
	...
	{
		$description['netbook_type_id'] = 'netbook_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM netbook  LEFT JOIN netbook_type ON netbook.netbook_type_id = netbook_type.netbook_type_id  LEFT JOIN netbook AS t ON netbook.netbook_id = t.netbook_id WHERE netbook.component_id = ? ORDER BY netbook.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['netbook_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['netbook_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM netbook  LEFT JOIN netbook_type ON netbook.netbook_type_id = netbook_type.netbook_type_id  LEFT JOIN netbook AS t ON netbook.netbook_id = t.netbook_id WHERE netbook.component_id = ? AND netbook.timestamp = (SELECT MAX(timestamp) FROM netbook WHERE netbook.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($netbook_id = null, $component_id = null, $netbook_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($netbook_id !== null)
		{
			self::$netbook_id = $netbook_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES netbook WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO netbook (netbook_id, component_id, netbook_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE netbook_id=VALUES(netbook_id),component_id=VALUES(component_id),netbook_type_id=VALUES(netbook_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->netbook_id, $this->component_id, $this->netbook_type_id, $this->timestamp, $this->youser_id);
