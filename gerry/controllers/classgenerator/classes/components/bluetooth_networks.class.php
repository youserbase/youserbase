<?php
class bluetooth_networks
{

	protected static $instance = null;
	private static $table = 'bluetooth_networks';
	public static $data = array();
	public $bluetooth_networks_id;
	public $component_id;
	public $component_id_int;
	public $bluetooth_networks_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($bluetooth_networks_id = null)
	{
		if($bluetooth_networks_id != null)
		{
			self::$bluetooth_networks_id = $bluetooth_networks_id;
		}
	...
	{
		$description['bluetooth_networks_type_id'] = 'bluetooth_networks_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM bluetooth_networks  LEFT JOIN bluetooth_networks_type ON bluetooth_networks.bluetooth_networks_type_id = bluetooth_networks_type.bluetooth_networks_type_id  LEFT JOIN bluetooth_networks AS t ON bluetooth_networks.bluetooth_networks_id = t.bluetooth_networks_id WHERE bluetooth_networks.component_id = ? ORDER BY bluetooth_networks.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['bluetooth_networks_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['bluetooth_networks_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM bluetooth_networks  LEFT JOIN bluetooth_networks_type ON bluetooth_networks.bluetooth_networks_type_id = bluetooth_networks_type.bluetooth_networks_type_id  LEFT JOIN bluetooth_networks AS t ON bluetooth_networks.bluetooth_networks_id = t.bluetooth_networks_id WHERE bluetooth_networks.component_id = ? AND bluetooth_networks.timestamp = (SELECT MAX(timestamp) FROM bluetooth_networks WHERE bluetooth_networks.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($bluetooth_networks_id = null, $component_id = null, $component_id_int = null, $bluetooth_networks_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($bluetooth_networks_id !== null)
		{
			self::$bluetooth_networks_id = $bluetooth_networks_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES bluetooth_networks WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO bluetooth_networks (bluetooth_networks_id, component_id, component_id_int, bluetooth_networks_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE bluetooth_networks_id=VALUES(bluetooth_networks_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),bluetooth_networks_type_id=VALUES(bluetooth_networks_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->bluetooth_networks_id, $this->component_id, $this->component_id_int, $this->bluetooth_networks_type_id, $this->timestamp, $this->youser_id);
