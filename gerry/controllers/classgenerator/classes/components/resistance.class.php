...
class resistance
{
	...
	private static $table = 'resistance';
	public static $data = array();
	public $resistance_id;
	public $component_id;
	public $component_id_int;
	public $resistance_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($resistance_id = null)
	{
		if($resistance_id != null)
		{
	...
	{
		$description['resistance_type_id'] = 'resistance_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM resistance  LEFT JOIN resistance_type ON resistance.resistance_type_id = resistance_type.resistance_type_id  LEFT JOIN resistance AS t ON resistance.resistance_id = t.resistance_id WHERE resistance.component_id = ? ORDER BY resistance.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['resistance_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['resistance_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM resistance  LEFT JOIN resistance_type ON resistance.resistance_type_id = resistance_type.resistance_type_id  LEFT JOIN resistance AS t ON resistance.resistance_id = t.resistance_id WHERE resistance.component_id = ? AND resistance.timestamp = (SELECT MAX(timestamp) FROM resistance WHERE resistance.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($resistance_id = null, $component_id = null, $component_id_int = null, $resistance_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($resistance_id !== null)
		{
			self::$resistance_id = $resistance_id;
		...
		}		if($resistance_type_id !== null)
		{
			self::$resistance_type_id = $resistance_type_id;
		}		if($timestamp !== null)
		{
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES resistance WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO resistance (resistance_id, component_id, component_id_int, resistance_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE resistance_id=VALUES(resistance_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),resistance_type_id=VALUES(resistance_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->resistance_id, $this->component_id, $this->component_id_int, $this->resistance_type_id, $this->timestamp, $this->youser_id);
