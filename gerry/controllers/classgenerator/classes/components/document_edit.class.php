...
class document_edit
{
	...
	private static $table = 'document_edit';
	public static $data = array();
	public $document_edit_id;
	public $component_id;
	public $component_id_int;
	public $document_format_type_id;
	public $document_format_type_id_int;
	public $timestamp;
	public $youser_id;
	...
	public function Get($document_edit_id = null)
	{
		if($document_edit_id != null)
		{
			self::$document_edit_id = $document_edit_id;
		}
	...
	{
		$description['document_format_type_id'] = 'document_format_type_id';
		$description['document_format_type_id_int'] = 'document_format_type_id_int';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM document_edit  LEFT JOIN document_format_type ON document_edit.document_format_type_id = document_format_type.document_format_type_id  LEFT JOIN document_edit AS t ON document_edit.document_edit_id = t.document_edit_id WHERE document_edit.component_id = ? ORDER BY document_edit.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['document_edit_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['document_format_type_id'] = 'multiple';
		$description['document_format_type_id_int'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM document_edit  LEFT JOIN document_format_type ON document_edit.document_format_type_id = document_format_type.document_format_type_id  LEFT JOIN document_edit AS t ON document_edit.document_edit_id = t.document_edit_id WHERE document_edit.component_id = ? AND document_edit.timestamp = (SELECT MAX(timestamp) FROM document_edit WHERE document_edit.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($document_edit_id = null, $component_id = null, $component_id_int = null, $document_format_type_id = null, $document_format_type_id_int = null, $timestamp = null, $youser_id = null)
	{
		if($document_edit_id !== null)
		{
			self::$document_edit_id = $document_edit_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES document_edit WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO document_edit (document_edit_id, component_id, component_id_int, document_format_type_id, document_format_type_id_int, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE document_edit_id=VALUES(document_edit_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),document_format_type_id=VALUES(document_format_type_id),document_format_type_id_int=VALUES(document_format_type_id_int),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->document_edit_id, $this->component_id, $this->component_id_int, $this->document_format_type_id, $this->document_format_type_id_int, $this->timestamp, $this->youser_id);
