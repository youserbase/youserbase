...
class exposure_modes
{
	...
	private static $table = 'exposure_modes';
	public static $data = array();
	public $exposure_modes;
	public $component_id;
	public $component_id_int;
	public $exposure_modes_type_id;
	public $exposure_modes_type_id_int;
	public $timestemp;
	public $youser_id;
	...
	public function Get($exposure_modes = null)
	{
		if($exposure_modes != null)
		{
			self::$exposure_modes = $exposure_modes;
		}
	...
	{
		$description['exposure_modes_type_id'] = 'exposure_modes_type_id';
		$description['exposure_modes_type_id_int'] = 'exposure_modes_type_id_int';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM exposure_modes  LEFT JOIN exposure_modes_type ON exposure_modes.exposure_modes_type_id = exposure_modes_type.exposure_modes_type_id  LEFT JOIN exposure_modes AS t ON exposure_modes.exposure_modes_id = t.exposure_modes_id WHERE exposure_modes.component_id = ? ORDER BY exposure_modes.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['exposure_modes'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['exposure_modes_type_id'] = 'multiple';
		$description['exposure_modes_type_id_int'] = 'multiple';
		$description['timestemp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM exposure_modes  LEFT JOIN exposure_modes_type ON exposure_modes.exposure_modes_type_id = exposure_modes_type.exposure_modes_type_id  LEFT JOIN exposure_modes AS t ON exposure_modes.exposure_modes_id = t.exposure_modes_id WHERE exposure_modes.component_id = ? AND exposure_modes.timestamp = (SELECT MAX(timestamp) FROM exposure_modes WHERE exposure_modes.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($exposure_modes = null, $component_id = null, $component_id_int = null, $exposure_modes_type_id = null, $exposure_modes_type_id_int = null, $timestemp = null, $youser_id = null)
	{
		if($exposure_modes !== null)
		{
			self::$exposure_modes = $exposure_modes;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES exposure_modes WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO exposure_modes (exposure_modes, component_id, component_id_int, exposure_modes_type_id, exposure_modes_type_id_int, timestemp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE exposure_modes=VALUES(exposure_modes),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),exposure_modes_type_id=VALUES(exposure_modes_type_id),exposure_modes_type_id_int=VALUES(exposure_modes_type_id_int),timestemp=VALUES(timestemp),youser_id=VALUES(youser_id);", $this->exposure_modes, $this->component_id, $this->component_id_int, $this->exposure_modes_type_id, $this->exposure_modes_type_id_int, $this->timestemp, $this->youser_id);
