...
class calculator_functions
{
	...
	private static $table = 'calculator_functions';
	public static $data = array();
	public $calculator_functions_id;
	public $component_id;
	public $component_id_int;
	public $calculator_functions_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($calculator_functions_id = null)
	{
		if($calculator_functions_id != null)
		{
			self::$calculator_functions_id = $calculator_functions_id;
		}
	...
	{
		$description['calculator_functions_type_id'] = 'calculator_functions_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM calculator_functions  LEFT JOIN calculator_functions_type ON calculator_functions.calculator_functions_type_id = calculator_functions_type.calculator_functions_type_id  LEFT JOIN calculator_functions AS t ON calculator_functions.calculator_functions_id = t.calculator_functions_id WHERE calculator_functions.component_id = ? ORDER BY calculator_functions.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['calculator_functions_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['calculator_functions_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM calculator_functions  LEFT JOIN calculator_functions_type ON calculator_functions.calculator_functions_type_id = calculator_functions_type.calculator_functions_type_id  LEFT JOIN calculator_functions AS t ON calculator_functions.calculator_functions_id = t.calculator_functions_id WHERE calculator_functions.component_id = ? AND calculator_functions.timestamp = (SELECT MAX(timestamp) FROM calculator_functions WHERE calculator_functions.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($calculator_functions_id = null, $component_id = null, $component_id_int = null, $calculator_functions_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($calculator_functions_id !== null)
		{
			self::$calculator_functions_id = $calculator_functions_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES calculator_functions WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO calculator_functions (calculator_functions_id, component_id, component_id_int, calculator_functions_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE calculator_functions_id=VALUES(calculator_functions_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),calculator_functions_type_id=VALUES(calculator_functions_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->calculator_functions_id, $this->component_id, $this->component_id_int, $this->calculator_functions_type_id, $this->timestamp, $this->youser_id);
