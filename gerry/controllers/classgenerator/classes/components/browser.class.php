...
class browser
{
	...
	private static $table = 'browser';
	public static $data = array();
	public $browser_id;
	public $component_id;
	public $component_id_int;
	public $browser_type_id;
	public $browser_type_id_int;
	public $timestamp;
	public $youser_id;
	...
	public function Get($browser_id = null)
	{
		if($browser_id != null)
		{
	...
	{
		$description['browser_type_id'] = 'browser_type_id';
		$description['browser_type_id_int'] = 'browser_type_id_int';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM browser  LEFT JOIN browser_type ON browser.browser_type_id = browser_type.browser_type_id  LEFT JOIN browser AS t ON browser.browser_id = t.browser_id WHERE browser.component_id = ? ORDER BY browser.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['browser_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['browser_type_id'] = 'multiple';
		$description['browser_type_id_int'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM browser  LEFT JOIN browser_type ON browser.browser_type_id = browser_type.browser_type_id  LEFT JOIN browser AS t ON browser.browser_id = t.browser_id WHERE browser.component_id = ? AND browser.timestamp = (SELECT MAX(timestamp) FROM browser WHERE browser.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($browser_id = null, $component_id = null, $component_id_int = null, $browser_type_id = null, $browser_type_id_int = null, $timestamp = null, $youser_id = null)
	{
		if($browser_id !== null)
		{
			self::$browser_id = $browser_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES browser WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO browser (browser_id, component_id, component_id_int, browser_type_id, browser_type_id_int, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE browser_id=VALUES(browser_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),browser_type_id=VALUES(browser_type_id),browser_type_id_int=VALUES(browser_type_id_int),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->browser_id, $this->component_id, $this->component_id_int, $this->browser_type_id, $this->browser_type_id_int, $this->timestamp, $this->youser_id);
