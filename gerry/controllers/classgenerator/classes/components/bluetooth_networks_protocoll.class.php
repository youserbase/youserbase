<?php
class bluetooth_networks_protocoll
{

	protected static $instance = null;
	private static $table = 'bluetooth_networks_protocoll';
	public static $data = array();
	public $bluetooth_networks_protocoll_id;
	public $component_id;
	public $component_id_int;
	public $bluetooth_networks_protocoll_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($bluetooth_networks_protocoll_id = null)
	{
		if($bluetooth_networks_protocoll_id != null)
		{
			self::$bluetooth_networks_protocoll_id = $bluetooth_networks_protocoll_id;
		}
	...
	{
		$description['bluetooth_networks_protocoll_type_id'] = 'bluetooth_networks_protocoll_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM bluetooth_networks_protocoll  LEFT JOIN bluetooth_networks_protocoll_type ON bluetooth_networks_protocoll.bluetooth_networks_protocoll_type_id = bluetooth_networks_protocoll_type.bluetooth_networks_protocoll_type_id  LEFT JOIN bluetooth_networks_protocoll AS t ON bluetooth_networks_protocoll.bluetooth_networks_protocoll_id = t.bluetooth_networks_protocoll_id WHERE bluetooth_networks_protocoll.component_id = ? ORDER BY bluetooth_networks_protocoll.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['bluetooth_networks_protocoll_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['bluetooth_networks_protocoll_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM bluetooth_networks_protocoll  LEFT JOIN bluetooth_networks_protocoll_type ON bluetooth_networks_protocoll.bluetooth_networks_protocoll_type_id = bluetooth_networks_protocoll_type.bluetooth_networks_protocoll_type_id  LEFT JOIN bluetooth_networks_protocoll AS t ON bluetooth_networks_protocoll.bluetooth_networks_protocoll_id = t.bluetooth_networks_protocoll_id WHERE bluetooth_networks_protocoll.component_id = ? AND bluetooth_networks_protocoll.timestamp = (SELECT MAX(timestamp) FROM bluetooth_networks_protocoll WHERE bluetooth_networks_protocoll.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($bluetooth_networks_protocoll_id = null, $component_id = null, $component_id_int = null, $bluetooth_networks_protocoll_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($bluetooth_networks_protocoll_id !== null)
		{
			self::$bluetooth_networks_protocoll_id = $bluetooth_networks_protocoll_id;
		...
		}		if($bluetooth_networks_protocoll_type_id !== null)
		{
			self::$bluetooth_networks_protocoll_type_id = $bluetooth_networks_protocoll_type_id;
		}		if($timestamp !== null)
		{
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES bluetooth_networks_protocoll WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO bluetooth_networks_protocoll (bluetooth_networks_protocoll_id, component_id, component_id_int, bluetooth_networks_protocoll_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE bluetooth_networks_protocoll_id=VALUES(bluetooth_networks_protocoll_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),bluetooth_networks_protocoll_type_id=VALUES(bluetooth_networks_protocoll_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->bluetooth_networks_protocoll_id, $this->component_id, $this->component_id_int, $this->bluetooth_networks_protocoll_type_id, $this->timestamp, $this->youser_id);
