...
class mail_limiter
{
	...
	private static $table = 'mail_limiter';
	...
	public function Get($youser_id = null)
	{
		if($youser_id != null)
		{
	...
	public function Loadview($component_id = null, $shift = 0)
	{
		...
		$description['mail_limit'] = 'mail_limit';
		$description['send_mails'] = 'send_mails';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM mail_limiter  WHERE mail_limiter.component_id = ? ORDER BY mail_limiter.timestamp DESC;", $component_id)->to_array();
	...
	public function Loadedit($component_id = null)
	{
		...
		$description['mail_limit'] = '';
		$description['send_mails'] = '';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM mail_limiter  WHERE mail_limiter.component_id = ? ORDER BY mail_limiter.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($youser_id = null, $mail_limit = null, $send_mails = null)
	{
		if($youser_id !== null)
		{
			self::$youser_id = $youser_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES mail_limiter WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO mail_limiter (youser_id, mail_limit, send_mails) VALUES(?, ?, ?) ON DUPLICATE KEY UPDATE youser_id=VALUES(youser_id),mail_limit=VALUES(mail_limit),send_mails=VALUES(send_mails);", $this->youser_id, $this->mail_limit, $this->send_mails);
