...
class flash
{
	...
	public $flash_guide_number;
	public $flash_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($flash_id = null)
	{
		if($flash_id != null)
		{
	...
	{
		$description['flash_reach'] = 'flash_reach';
		$description['flash_guide_number'] = 'flash_guide_number';
		$description['flash_type_id'] = 'flash_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM flash  LEFT JOIN time_units_type ON flash.time_units_type_id = time_units_type.time_units_type_id  LEFT JOIN flash_type ON flash.flash_type_id = flash_type.flash_type_id  LEFT JOIN flash AS t ON flash.flash_id = t.flash_id WHERE flash.component_id = ? ORDER BY flash.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['flash_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['flash_reach'] = 'text';
		$description['time_units_type_id'] = 'notshown';
		$description['flash_guide_number'] = 'text';
		$description['flash_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM flash  LEFT JOIN time_units_type ON flash.time_units_type_id = time_units_type.time_units_type_id  LEFT JOIN flash_type ON flash.flash_type_id = flash_type.flash_type_id  LEFT JOIN flash AS t ON flash.flash_id = t.flash_id WHERE flash.component_id = ? AND flash.timestamp = (SELECT MAX(timestamp) FROM flash WHERE flash.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($flash_id = null, $component_id = null, $component_id_int = null, $flash_reach = null, $time_units_type_id = null, $flash_guide_number = null, $flash_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($flash_id !== null)
		{
			self::$flash_id = $flash_id;
		...
		{
			self::$flash_reach = $flash_reach;
		}		if($time_units_type_id !== null)
		{
			self::$time_units_type_id = $time_units_type_id;
		}		if($flash_guide_number !== null)
		{
			self::$flash_guide_number = $flash_guide_number;
		}		if($flash_type_id !== null)
		{
			self::$flash_type_id = $flash_type_id;
		}		if($timestamp !== null)
		{
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES flash WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO flash (flash_id, component_id, component_id_int, flash_reach, time_units_type_id, flash_guide_number, flash_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE flash_id=VALUES(flash_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),flash_reach=VALUES(flash_reach),time_units_type_id=VALUES(time_units_type_id),flash_guide_number=VALUES(flash_guide_number),flash_type_id=VALUES(flash_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->flash_id, $this->component_id, $this->component_id_int, $this->flash_reach, $this->time_units_type_id, $this->flash_guide_number, $this->flash_type_id, $this->timestamp, $this->youser_id);
