...
class nav_routing
{
	...
	private static $table = 'nav_routing';
	public static $data = array();
	public $nav_routing_id;
	public $component_id;
	public $component_id_int;
	public $nav_routing_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($nav_routing_id = null)
	{
		if($nav_routing_id != null)
		{
			self::$nav_routing_id = $nav_routing_id;
		}
	...
	{
		$description['nav_routing_type_id'] = 'nav_routing_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM nav_routing  LEFT JOIN nav_routing_type ON nav_routing.nav_routing_type_id = nav_routing_type.nav_routing_type_id  LEFT JOIN nav_routing AS t ON nav_routing.nav_routing_id = t.nav_routing_id WHERE nav_routing.component_id = ? ORDER BY nav_routing.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['nav_routing_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['nav_routing_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM nav_routing  LEFT JOIN nav_routing_type ON nav_routing.nav_routing_type_id = nav_routing_type.nav_routing_type_id  LEFT JOIN nav_routing AS t ON nav_routing.nav_routing_id = t.nav_routing_id WHERE nav_routing.component_id = ? AND nav_routing.timestamp = (SELECT MAX(timestamp) FROM nav_routing WHERE nav_routing.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($nav_routing_id = null, $component_id = null, $component_id_int = null, $nav_routing_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($nav_routing_id !== null)
		{
			self::$nav_routing_id = $nav_routing_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES nav_routing WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO nav_routing (nav_routing_id, component_id, component_id_int, nav_routing_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE nav_routing_id=VALUES(nav_routing_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),nav_routing_type_id=VALUES(nav_routing_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->nav_routing_id, $this->component_id, $this->component_id_int, $this->nav_routing_type_id, $this->timestamp, $this->youser_id);
