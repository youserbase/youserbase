...
class hspa_networks
{
	...
	private static $table = 'hspa_networks';
	public static $data = array();
	public $hspa_networks_id;
	public $component_id;
	public $component_id_int;
	public $hspa_networks_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($hspa_networks_id = null)
	{
		if($hspa_networks_id != null)
		{
			self::$hspa_networks_id = $hspa_networks_id;
		}
	...
	{
		$description['hspa_networks_type_id'] = 'hspa_networks_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM hspa_networks  LEFT JOIN hspa_networks_type ON hspa_networks.hspa_networks_type_id = hspa_networks_type.hspa_networks_type_id  LEFT JOIN hspa_networks AS t ON hspa_networks.hspa_networks_id = t.hspa_networks_id WHERE hspa_networks.component_id = ? ORDER BY hspa_networks.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['hspa_networks_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['hspa_networks_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM hspa_networks  LEFT JOIN hspa_networks_type ON hspa_networks.hspa_networks_type_id = hspa_networks_type.hspa_networks_type_id  LEFT JOIN hspa_networks AS t ON hspa_networks.hspa_networks_id = t.hspa_networks_id WHERE hspa_networks.component_id = ? AND hspa_networks.timestamp = (SELECT MAX(timestamp) FROM hspa_networks WHERE hspa_networks.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($hspa_networks_id = null, $component_id = null, $component_id_int = null, $hspa_networks_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($hspa_networks_id !== null)
		{
			self::$hspa_networks_id = $hspa_networks_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES hspa_networks WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO hspa_networks (hspa_networks_id, component_id, component_id_int, hspa_networks_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE hspa_networks_id=VALUES(hspa_networks_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),hspa_networks_type_id=VALUES(hspa_networks_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->hspa_networks_id, $this->component_id, $this->component_id_int, $this->hspa_networks_type_id, $this->timestamp, $this->youser_id);
