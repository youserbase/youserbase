<?php
class nav_routing_avoid
{

	protected static $instance = null;
	private static $table = 'nav_routing_avoid';
	public static $data = array();
	public $nav_routing_avoid_id;
	public $component_id;
	public $component_id_int;
	public $nav_routing_avoid_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($nav_routing_avoid_id = null)
	{
		if($nav_routing_avoid_id != null)
		{
			self::$nav_routing_avoid_id = $nav_routing_avoid_id;
		}
	...
	{
		$description['nav_routing_avoid_type_id'] = 'nav_routing_avoid_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM nav_routing_avoid  LEFT JOIN nav_routing_avoid_type ON nav_routing_avoid.nav_routing_avoid_type_id = nav_routing_avoid_type.nav_routing_avoid_type_id  LEFT JOIN nav_routing_avoid AS t ON nav_routing_avoid.nav_routing_avoid_id = t.nav_routing_avoid_id WHERE nav_routing_avoid.component_id = ? ORDER BY nav_routing_avoid.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['nav_routing_avoid_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['nav_routing_avoid_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM nav_routing_avoid  LEFT JOIN nav_routing_avoid_type ON nav_routing_avoid.nav_routing_avoid_type_id = nav_routing_avoid_type.nav_routing_avoid_type_id  LEFT JOIN nav_routing_avoid AS t ON nav_routing_avoid.nav_routing_avoid_id = t.nav_routing_avoid_id WHERE nav_routing_avoid.component_id = ? AND nav_routing_avoid.timestamp = (SELECT MAX(timestamp) FROM nav_routing_avoid WHERE nav_routing_avoid.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($nav_routing_avoid_id = null, $component_id = null, $component_id_int = null, $nav_routing_avoid_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($nav_routing_avoid_id !== null)
		{
			self::$nav_routing_avoid_id = $nav_routing_avoid_id;
		...
		}		if($nav_routing_avoid_type_id !== null)
		{
			self::$nav_routing_avoid_type_id = $nav_routing_avoid_type_id;
		}		if($timestamp !== null)
		{
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES nav_routing_avoid WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO nav_routing_avoid (nav_routing_avoid_id, component_id, component_id_int, nav_routing_avoid_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE nav_routing_avoid_id=VALUES(nav_routing_avoid_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),nav_routing_avoid_type_id=VALUES(nav_routing_avoid_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->nav_routing_avoid_id, $this->component_id, $this->component_id_int, $this->nav_routing_avoid_type_id, $this->timestamp, $this->youser_id);
