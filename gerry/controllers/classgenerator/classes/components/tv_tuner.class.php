...
class tv_tuner
{
	...
	private static $table = 'tv_tuner';
	public static $data = array();
	public $tv_tuner_id;
	public $component_id;
	public $component_id_int;
	public $tv_tuner_type_id;
	public $timestamp;
	public $youser_id;
	...
	public function Get($tv_tuner_id = null)
	{
		if($tv_tuner_id != null)
		{
			self::$tv_tuner_id = $tv_tuner_id;
		}
	...
	{
		$description['tv_tuner_type_id'] = 'tv_tuner_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM tv_tuner  LEFT JOIN tv_tuner_type ON tv_tuner.tv_tuner_type_id = tv_tuner_type.tv_tuner_type_id  LEFT JOIN tv_tuner AS t ON tv_tuner.tv_tuner_id = t.tv_tuner_id WHERE tv_tuner.component_id = ? ORDER BY tv_tuner.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['tv_tuner_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['tv_tuner_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM tv_tuner  LEFT JOIN tv_tuner_type ON tv_tuner.tv_tuner_type_id = tv_tuner_type.tv_tuner_type_id  LEFT JOIN tv_tuner AS t ON tv_tuner.tv_tuner_id = t.tv_tuner_id WHERE tv_tuner.component_id = ? AND tv_tuner.timestamp = (SELECT MAX(timestamp) FROM tv_tuner WHERE tv_tuner.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($tv_tuner_id = null, $component_id = null, $component_id_int = null, $tv_tuner_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($tv_tuner_id !== null)
		{
			self::$tv_tuner_id = $tv_tuner_id;
		...
		}		if($tv_tuner_type_id !== null)
		{
			self::$tv_tuner_type_id = $tv_tuner_type_id;
		}		if($timestamp !== null)
		{
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES tv_tuner WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO tv_tuner (tv_tuner_id, component_id, component_id_int, tv_tuner_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE tv_tuner_id=VALUES(tv_tuner_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),tv_tuner_type_id=VALUES(tv_tuner_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->tv_tuner_id, $this->component_id, $this->component_id_int, $this->tv_tuner_type_id, $this->timestamp, $this->youser_id);
