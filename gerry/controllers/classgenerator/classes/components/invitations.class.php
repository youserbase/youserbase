...
class invitations
{
	...
	public function Get($invitation_id = null)
	{
	...
	{
		$description['invitation_id'] = 'invitation_id';
		$description['youser_id'] = 'youser_id';
		$description['invited_mail'] = 'invited_mail';
		$description['timestamp'] = 'timestamp';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM invitations  WHERE invitations.component_id = ? ORDER BY invitations.timestamp DESC;", $component_id)->to_array();
	...
	public function Loadedit($component_id = null)
	{
		...
		$description['invited_mail'] = '';
		$description['timestamp'] = '';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM invitations  WHERE invitations.component_id = ? ORDER BY invitations.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($invitation_id = null, $youser_id = null, $invited_mail = null, $timestamp = null)
	{
		if($invitation_id !== null)
		{
			self::$invitation_id = $invitation_id;
		...
		}		if($invited_mail !== null)
		{
			self::$invited_mail = $invited_mail;
		}		if($timestamp !== null)
		{
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES invitations WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO invitations (invitation_id, youser_id, invited_mail, timestamp) VALUES(?, ?, ?, ?) ON DUPLICATE KEY UPDATE invitation_id=VALUES(invitation_id),youser_id=VALUES(youser_id),invited_mail=VALUES(invited_mail),timestamp=VALUES(timestamp);", $this->invitation_id, $this->youser_id, $this->invited_mail, $this->timestamp);
