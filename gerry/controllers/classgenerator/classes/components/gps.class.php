...
class gps
{
	...
	private static $table = 'gps';
	public static $data = array();
	public $gps_id;
	public $component_id;
	public $component_id_int;
	public $number_of_satellites;
	...
	public function Get($gps_id = null)
	{
		if($gps_id != null)
		{
			self::$gps_id = $gps_id;
	...
	{
		$description['number_of_satellites'] = 'number_of_satellites';
		$description['input_method_type_id'] = 'input_method_type_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM gps  LEFT JOIN input_method_type ON gps.input_method_type_id = input_method_type.input_method_type_id  LEFT JOIN gps AS t ON gps.gps_id = t.gps_id WHERE gps.component_id = ? ORDER BY gps.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['gps_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['number_of_satellites'] = 'text';
		$description['input_method_type_id'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM gps  LEFT JOIN input_method_type ON gps.input_method_type_id = input_method_type.input_method_type_id  LEFT JOIN gps AS t ON gps.gps_id = t.gps_id WHERE gps.component_id = ? AND gps.timestamp = (SELECT MAX(timestamp) FROM gps WHERE gps.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($gps_id = null, $component_id = null, $component_id_int = null, $number_of_satellites = null, $input_method_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($gps_id !== null)
		{
			self::$gps_id = $gps_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES gps WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO gps (gps_id, component_id, component_id_int, number_of_satellites, input_method_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE gps_id=VALUES(gps_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),number_of_satellites=VALUES(number_of_satellites),input_method_type_id=VALUES(input_method_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->gps_id, $this->component_id, $this->component_id_int, $this->number_of_satellites, $this->input_method_type_id, $this->timestamp, $this->youser_id);
