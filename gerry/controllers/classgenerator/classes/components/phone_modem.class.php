...
class phone_modem
{
	...
	private static $table = 'phone_modem';
	public static $data = array();
	public $phone_modem_id;
	public $component_id;
	public $component_id_int;
	public $phone_modem_type_id;
	public $phone_modem_type_id_int;
	public $timestamp;
	public $youser_id;
	...
	public function Get($phone_modem_id = null)
	{
		if($phone_modem_id != null)
		{
			self::$phone_modem_id = $phone_modem_id;
		}
	...
	{
		$description['phone_modem_type_id'] = 'phone_modem_type_id';
		$description['phone_modem_type_id_int'] = 'phone_modem_type_id_int';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM phone_modem  LEFT JOIN phone_modem_type ON phone_modem.phone_modem_type_id = phone_modem_type.phone_modem_type_id  LEFT JOIN phone_modem AS t ON phone_modem.phone_modem_id = t.phone_modem_id WHERE phone_modem.component_id = ? ORDER BY phone_modem.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['phone_modem_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['phone_modem_type_id'] = 'multiple';
		$description['phone_modem_type_id_int'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM phone_modem  LEFT JOIN phone_modem_type ON phone_modem.phone_modem_type_id = phone_modem_type.phone_modem_type_id  LEFT JOIN phone_modem AS t ON phone_modem.phone_modem_id = t.phone_modem_id WHERE phone_modem.component_id = ? AND phone_modem.timestamp = (SELECT MAX(timestamp) FROM phone_modem WHERE phone_modem.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($phone_modem_id = null, $component_id = null, $component_id_int = null, $phone_modem_type_id = null, $phone_modem_type_id_int = null, $timestamp = null, $youser_id = null)
	{
		if($phone_modem_id !== null)
		{
			self::$phone_modem_id = $phone_modem_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES phone_modem WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO phone_modem (phone_modem_id, component_id, component_id_int, phone_modem_type_id, phone_modem_type_id_int, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE phone_modem_id=VALUES(phone_modem_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),phone_modem_type_id=VALUES(phone_modem_type_id),phone_modem_type_id_int=VALUES(phone_modem_type_id_int),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->phone_modem_id, $this->component_id, $this->component_id_int, $this->phone_modem_type_id, $this->phone_modem_type_id_int, $this->timestamp, $this->youser_id);
