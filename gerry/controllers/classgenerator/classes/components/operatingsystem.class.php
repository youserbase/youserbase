...
class operatingsystem
{
	...
	private static $table = 'operatingsystem';
	public static $data = array();
	public $operatingsystem_id;
	public $component_id;
	public $component_id_int;
	public $operatingsystem_type_id;
	public $operatingsystem_type_id_int;
	public $timestamp;
	public $youser_id;
	...
	public function Get($operatingsystem_id = null)
	{
		if($operatingsystem_id != null)
		{
	...
	{
		$description['operatingsystem_type_id'] = 'operatingsystem_type_id';
		$description['operatingsystem_type_id_int'] = 'operatingsystem_type_id_int';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM operatingsystem  LEFT JOIN operatingsystem_type ON operatingsystem.operatingsystem_type_id = operatingsystem_type.operatingsystem_type_id  LEFT JOIN operatingsystem AS t ON operatingsystem.operatingsystem_id = t.operatingsystem_id WHERE operatingsystem.component_id = ? ORDER BY operatingsystem.timestamp DESC;", $component_id)->to_array();
			...
			foreach($content as $line_name => $line_content)
			{
				if(strpos($line_name, '_id') === false && strpos($line_name, 'units') === false && strpos($line_name, 'memory_size_type') === false && strpos($line_name, 'currency_type_name') === false && strpos($line_name,'version') === false && strpos($line_name,'device') === false && strpos($line_name,'alternative') === false && strpos($line_name,'short') === false && strpos($line_name,'website') === false)
				{
					if(preg_match('/((\w+)?(_type))(_name\b)/', $line_name, $match))
					{
						...
						{
							if($line_name == 'operatingsystem_type_name')
							{
	...
	{
		$description['operatingsystem_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['operatingsystem_type_id'] = 'select';
		$description['operatingsystem_type_id_int'] = 'select';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';
		$description['device_type_name'] = 'hidden';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM operatingsystem  LEFT JOIN operatingsystem_type ON operatingsystem.operatingsystem_type_id = operatingsystem_type.operatingsystem_type_id  LEFT JOIN operatingsystem AS t ON operatingsystem.operatingsystem_id = t.operatingsystem_id WHERE operatingsystem.component_id = ? ORDER BY operatingsystem.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
		...
		if(isset($_REQUEST['device_id']))
		{
			...
			if($device_type == 'NETBOOK' || $device_type == 'TABLET'){
	...
	public function Set($operatingsystem_id = null, $component_id = null, $component_id_int = null, $operatingsystem_type_id = null, $operatingsystem_type_id_int = null, $timestamp = null, $youser_id = null, $device_type_name = null)
	{
		if($operatingsystem_id !== null)
		{
			self::$operatingsystem_id = $operatingsystem_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES operatingsystem WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO operatingsystem (operatingsystem_id, component_id, component_id_int, operatingsystem_type_id, operatingsystem_type_id_int, timestamp, youser_id, device_type_name) VALUES(?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE operatingsystem_id=VALUES(operatingsystem_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),operatingsystem_type_id=VALUES(operatingsystem_type_id),operatingsystem_type_id_int=VALUES(operatingsystem_type_id_int),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id),device_type_name=VALUES(device_type_name);", $this->operatingsystem_id, $this->component_id, $this->component_id_int, $this->operatingsystem_type_id, $this->operatingsystem_type_id_int, $this->timestamp, $this->youser_id, $this->device_type_name);
