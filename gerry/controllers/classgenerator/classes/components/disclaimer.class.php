...
class disclaimer
{
	...
	private static $table = 'disclaimer';
	public static $data = array();
	public $disclaimer_id;
	...
	public function Get($disclaimer_id = null)
	{
		if($disclaimer_id != null)
		{
			self::$disclaimer_id = $disclaimer_id;
		}
	...
	{
		$description['disclaimer_text'] = 'disclaimer_text';
		$description['language_id'] = 'language_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM disclaimer  WHERE disclaimer.component_id = ? ORDER BY disclaimer.timestamp DESC;", $component_id)->to_array();
				...
				if(strpos($line_name, '_id') === false && strpos($line_name, 'units') === false && strpos($line_name, 'memory_size_type') === false && strpos($line_name, 'currency_type_name') === false && strpos($line_name,'version') === false && strpos($line_name,'device') === false && strpos($line_name,'alternative') === false && strpos($line_name,'short') === false && strpos($line_name,'website') === false)
				{
					if(preg_match('/((\w+)?(_type))(_name\b)/', $line_name, $match))
					{
						if(isset($content[$match[1].'_name']))
						{
							...
							else if(is_array($content[$match[1].'_name']) && !in_array($content[$match[1].'_name'], $data[$match[1].'_id']))
							{
	...
	{
		$description['disclaimer_id'] = 'hidden';
		$description['disclaimer_text'] = 'text';
		$description['language_id'] = '';
		$description['youser_id'] = 'notshown';
		$description['timestamp'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM disclaimer  WHERE disclaimer.component_id = ? ORDER BY disclaimer.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($disclaimer_id = null, $disclaimer_text = null, $language_id = null, $youser_id = null, $timestamp = null)
	{
		if($disclaimer_id !== null)
		{
			self::$disclaimer_id = $disclaimer_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES disclaimer WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO disclaimer (disclaimer_id, disclaimer_text, language_id, youser_id, timestamp) VALUES(?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE disclaimer_id=VALUES(disclaimer_id),disclaimer_text=VALUES(disclaimer_text),language_id=VALUES(language_id),youser_id=VALUES(youser_id),timestamp=VALUES(timestamp);", $this->disclaimer_id, $this->disclaimer_text, $this->language_id, $this->youser_id, $this->timestamp);
