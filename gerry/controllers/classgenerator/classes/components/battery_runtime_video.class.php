<?php
class battery_runtime_video
{

	protected static $instance = null;
	private static $table = 'battery_runtime_video';
	public static $data = array();
	public $battery_runtime_video_id;
	public $component_id;
	public $component_id_int;
	public $battery_runtime_video_playback;
	public $battery_runtime_video_recording;
	...
	public function Get($battery_runtime_video_id = null)
	{
		if($battery_runtime_video_id != null)
		{
			self::$battery_runtime_video_id = $battery_runtime_video_id;
		}
	...
	{
		$description['battery_runtime_video_playback'] = 'battery_runtime_video_playback';
		$description['battery_runtime_video_recording'] = 'battery_runtime_video_recording';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM battery_runtime_video  LEFT JOIN time_units_type ON battery_runtime_video.time_units_type_id = time_units_type.time_units_type_id  LEFT JOIN battery_runtime_video AS t ON battery_runtime_video.battery_runtime_video_id = t.battery_runtime_video_id WHERE battery_runtime_video.component_id = ? ORDER BY battery_runtime_video.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['battery_runtime_video_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['battery_runtime_video_playback'] = 'text';
		$description['battery_runtime_video_recording'] = 'text';
		$description['time_units_type_id'] = 'notshown';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM battery_runtime_video  LEFT JOIN time_units_type ON battery_runtime_video.time_units_type_id = time_units_type.time_units_type_id  LEFT JOIN battery_runtime_video AS t ON battery_runtime_video.battery_runtime_video_id = t.battery_runtime_video_id WHERE battery_runtime_video.component_id = ? ORDER BY battery_runtime_video.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($battery_runtime_video_id = null, $component_id = null, $component_id_int = null, $battery_runtime_video_playback = null, $battery_runtime_video_recording = null, $time_units_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($battery_runtime_video_id !== null)
		{
			self::$battery_runtime_video_id = $battery_runtime_video_id;
		...
		}		if($battery_runtime_video_playback !== null)
		{
			self::$battery_runtime_video_playback = $battery_runtime_video_playback;
		}		if($battery_runtime_video_recording !== null)
		{
			self::$battery_runtime_video_recording = $battery_runtime_video_recording;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES battery_runtime_video WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO battery_runtime_video (battery_runtime_video_id, component_id, component_id_int, battery_runtime_video_playback, battery_runtime_video_recording, time_units_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE battery_runtime_video_id=VALUES(battery_runtime_video_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),battery_runtime_video_playback=VALUES(battery_runtime_video_playback),battery_runtime_video_recording=VALUES(battery_runtime_video_recording),time_units_type_id=VALUES(time_units_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->battery_runtime_video_id, $this->component_id, $this->component_id_int, $this->battery_runtime_video_playback, $this->battery_runtime_video_recording, $this->time_units_type_id, $this->timestamp, $this->youser_id);
