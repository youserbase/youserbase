...
class zoom
{
	...
	private static $table = 'zoom';
	public static $data = array();
	public $zoom_id;
	public $component_id;
	public $component_id_int;
	public $zoom_type_id;
	public $zoom_type_id_int;
	public $timestamp;
	public $youser_id;
	...
	public function Get($zoom_id = null)
	{
		if($zoom_id != null)
		{
	...
	{
		$description['zoom_type_id'] = 'zoom_type_id';
		$description['zoom_type_id_int'] = 'zoom_type_id_int';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM zoom  LEFT JOIN zoom_type ON zoom.zoom_type_id = zoom_type.zoom_type_id  LEFT JOIN zoom AS t ON zoom.zoom_id = t.zoom_id WHERE zoom.component_id = ? ORDER BY zoom.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['zoom_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['zoom_type_id'] = 'multiple';
		$description['zoom_type_id_int'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM zoom  LEFT JOIN zoom_type ON zoom.zoom_type_id = zoom_type.zoom_type_id  LEFT JOIN zoom AS t ON zoom.zoom_id = t.zoom_id WHERE zoom.component_id = ? AND zoom.timestamp = (SELECT MAX(timestamp) FROM zoom WHERE zoom.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($zoom_id = null, $component_id = null, $component_id_int = null, $zoom_type_id = null, $zoom_type_id_int = null, $timestamp = null, $youser_id = null)
	{
		if($zoom_id !== null)
		{
			self::$zoom_id = $zoom_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES zoom WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO zoom (zoom_id, component_id, component_id_int, zoom_type_id, zoom_type_id_int, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE zoom_id=VALUES(zoom_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),zoom_type_id=VALUES(zoom_type_id),zoom_type_id_int=VALUES(zoom_type_id_int),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->zoom_id, $this->component_id, $this->component_id_int, $this->zoom_type_id, $this->zoom_type_id_int, $this->timestamp, $this->youser_id);
