...
class countrylanguages
{
	...
	private static $table = 'countrylanguages';
	...
	public function Get($cl_id = null)
	{
		if($cl_id != null)
		{
			self::$cl_id = $cl_id;
	...
	{
		$description['cl_id'] = 'cl_id';
		$description['language_id'] = 'language_id';
		$description['country_id'] = 'country_id';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM countrylanguages  WHERE countrylanguages.component_id = ? ORDER BY countrylanguages.timestamp DESC;", $component_id)->to_array();
	...
	public function Loadedit($component_id = null)
	{
		...
		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM countrylanguages  WHERE countrylanguages.component_id = ? ORDER BY countrylanguages.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($cl_id = null, $language_id = null, $country_id = null, $timestamp = null, $youser_id = null)
	{
		if($cl_id !== null)
		{
			self::$cl_id = $cl_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES countrylanguages WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO countrylanguages (cl_id, language_id, country_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE cl_id=VALUES(cl_id),language_id=VALUES(language_id),country_id=VALUES(country_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->cl_id, $this->language_id, $this->country_id, $this->timestamp, $this->youser_id);
