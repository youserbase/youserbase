...
class ram
{
	...
	public function Get($ram_id = null)
	{
		if($ram_id != null)
		{
			self::$ram_id = $ram_id;
	...
	{
		$description['ram_size'] = 'ram_size';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM ram  LEFT JOIN memory_size_type ON ram.memory_size_type_id = memory_size_type.memory_size_type_id  LEFT JOIN ram AS t ON ram.ram_id = t.ram_id WHERE ram.component_id = ? ORDER BY ram.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['ram_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['ram_size'] = 'text';
		$description['memory_size_type_id'] = 'notshown';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM ram  LEFT JOIN memory_size_type ON ram.memory_size_type_id = memory_size_type.memory_size_type_id  LEFT JOIN ram AS t ON ram.ram_id = t.ram_id WHERE ram.component_id = ? ORDER BY ram.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($ram_id = null, $component_id = null, $component_id_int = null, $ram_size = null, $memory_size_type_id = null, $timestamp = null, $youser_id = null)
	{
		if($ram_id !== null)
		{
			self::$ram_id = $ram_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES ram WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO ram (ram_id, component_id, component_id_int, ram_size, memory_size_type_id, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE ram_id=VALUES(ram_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),ram_size=VALUES(ram_size),memory_size_type_id=VALUES(memory_size_type_id),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->ram_id, $this->component_id, $this->component_id_int, $this->ram_size, $this->memory_size_type_id, $this->timestamp, $this->youser_id);
