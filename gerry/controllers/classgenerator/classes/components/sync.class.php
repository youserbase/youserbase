...
class sync
{
	...
	private static $table = 'sync';
	public static $data = array();
	public $sync_id;
	public $component_id;
	public $component_id_int;
	public $sync_type_id;
	public $sync_type_id_int;
	public $timestamp;
	public $youser_id;
	...
	public function Get($sync_id = null)
	{
		if($sync_id != null)
		{
	...
	{
		$description['sync_type_id'] = 'sync_type_id';
		$description['sync_type_id_int'] = 'sync_type_id_int';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM sync  LEFT JOIN sync_type ON sync.sync_type_id = sync_type.sync_type_id  LEFT JOIN sync AS t ON sync.sync_id = t.sync_id WHERE sync.component_id = ? ORDER BY sync.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['sync_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['sync_type_id'] = 'multiple';
		$description['sync_type_id_int'] = 'multiple';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM sync  LEFT JOIN sync_type ON sync.sync_type_id = sync_type.sync_type_id  LEFT JOIN sync AS t ON sync.sync_id = t.sync_id WHERE sync.component_id = ? AND sync.timestamp = (SELECT MAX(timestamp) FROM sync WHERE sync.component_id = ?);", $component_id, $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($sync_id = null, $component_id = null, $component_id_int = null, $sync_type_id = null, $sync_type_id_int = null, $timestamp = null, $youser_id = null)
	{
		if($sync_id !== null)
		{
			self::$sync_id = $sync_id;
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES sync WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO sync (sync_id, component_id, component_id_int, sync_type_id, sync_type_id_int, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE sync_id=VALUES(sync_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),sync_type_id=VALUES(sync_type_id),sync_type_id_int=VALUES(sync_type_id_int),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->sync_id, $this->component_id, $this->component_id_int, $this->sync_type_id, $this->sync_type_id_int, $this->timestamp, $this->youser_id);
