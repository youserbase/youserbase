...
class radiation
{
	...
	private static $table = 'radiation';
	public static $data = array();
	public $radiation_id;
	public $component_id;
	public $component_id_int;
	public $radiation_sar;
	public $timestamp;
	public $youser_id;
	...
	public function Get($radiation_id = null)
	{
		if($radiation_id != null)
		{
			self::$radiation_id = $radiation_id;
		}
	...
	{
		$description['radiation_sar'] = 'radiation_sar';
		if($component_id == null) return $description;

		$response = DBManager::Get('devices')->query("SELECT * FROM radiation  WHERE radiation.component_id = ? ORDER BY radiation.timestamp DESC;", $component_id)->to_array();
	...
	{
		$description['radiation_id'] = 'hidden';
		$description['component_id'] = 'hidden';
		$description['component_id_int'] = 'hidden';
		$description['radiation_sar'] = 'text';
		$description['timestamp'] = 'notshown';
		$description['youser_id'] = 'notshown';

		$data = $description;
		$result = DBManager::Get('devices')->query("SELECT * FROM radiation  WHERE radiation.component_id = ? ORDER BY radiation.timestamp DESC LIMIT 0,1;", $component_id)->to_array();
		if($result !== null){
			$data = array();
	...
	public function Set($radiation_id = null, $component_id = null, $component_id_int = null, $radiation_sar = null, $timestamp = null, $youser_id = null)
	{
		if($radiation_id !== null)
		{
			self::$radiation_id = $radiation_id;
		...
		}		if($radiation_sar !== null)
		{
			self::$radiation_sar = $radiation_sar;
		}		if($timestamp !== null)
		{
	...
	{
		DBManager::Get('devices')->query("LOCK TABLES radiation WRITE;");
		$db = DBManager::Get('devices')->query("INSERT INTO radiation (radiation_id, component_id, component_id_int, radiation_sar, timestamp, youser_id) VALUES(?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE radiation_id=VALUES(radiation_id),component_id=VALUES(component_id),component_id_int=VALUES(component_id_int),radiation_sar=VALUES(radiation_sar),timestamp=VALUES(timestamp),youser_id=VALUES(youser_id);", $this->radiation_id, $this->component_id, $this->component_id_int, $this->radiation_sar, $this->timestamp, $this->youser_id);
